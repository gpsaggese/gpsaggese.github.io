...
def plot_forecast(
    ...
    plt.title(
        f"{model_name} Forecast Visualization", fontsize=16, fontweight="bold"
    )
...
def plot_error_analysis(
    ...
    mape = np.mean(ape)
    axes[1, 0].bar(
        range(1, forecast_period + 1), ape, color="steelblue", alpha=0.7
    )
    axes[1, 0].axhline(
        y=mape,
        color="red",
    ...
    if 0.1 in forecast_quantiles and 0.9 in forecast_quantiles:
        ...
        axes[1, 1].set_title(
            "Forecast Uncertainty (80% CI Width)", fontweight="bold"
        )
    ...
    plt.suptitle(
        f"{model_name} Error Analysis", fontsize=16, fontweight="bold", y=1.00
    )
    plt.tight_layout()
...
def compare_models(
    results: Dict[str, Dict[str, float]], save_path: str = None
) -> None:
    ...
    for model, metrics in results.items():
        print(
            f"{model:<20} {metrics['mae']:>12,.2f} {metrics['rmse']:>12,.2f} {metrics['mape']:>11.2f}%"
        )
    print("=" * 70)
