...
dataset_path = "/Users/asutoshdalei/Desktop/Masters/Padhai/MSML610 - AdvML/CLIPEmbed/flickr8k/"
...
with tab1:
    ...
    st.write("Text → Image Retrieval: Input a text query and retrieve the top 3 most relevant images from a dataset")
    st.write(
...
with tab2:
    k_ = st.segmented_control("Select k number", options=[1,2,3,4,5],default=1, selection_mode="single", key='txt2img')
    ...
    if input_promt_text:
        ...
        # t1, t2, t3 = st.columns(k_, vertical_alignment="top")
        # with t1:
...
with tab3:
    ...
    k_ = st.segmented_control("Select k number", options=[1,2,3,4,5], default=1, selection_mode="single",key='img2txt')
    if input_promt_image:
        ...
        for i in range(k_):
            ...
            st.container(border=True).write(f"{row} \n | Hit Distance: {distances[0][i]}")
