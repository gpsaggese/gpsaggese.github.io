...
This document explains the **complete example application** implemented in `auto_sklearn.example.ipynb`.
...
Below is a **section-level explanation** of what the notebook does, what it produces, and why it exists.
...
**Why this matters**
The EDA supports the decision to use **non-linear/ensemble models** (auto-sklearn + tree baselines) rather than assuming a strong linear signal.
...
**Purpose**: train models and compare performance fairly.
...
- Auto-sklearn training statistics (best validation score, run counts, etc.).
