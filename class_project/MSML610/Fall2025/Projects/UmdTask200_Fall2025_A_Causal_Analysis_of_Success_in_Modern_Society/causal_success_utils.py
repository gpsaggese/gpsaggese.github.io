...
class Agent:
    ...
    Attributes:
        ...
        talent: dict of talent dimensions (intensity, iq, networking, initial_capital)
        capital: Current wealth level
        capital_history: List of capital values over time
        lucky_events: Count of beneficial events received
        unlucky_events: Count of detrimental events received
    """
    ...
    def apply_event(self, event_type: str, impact: float) -> None:
        ...
        Args:
            event_type: "lucky" or "unlucky"
            impact: magnitude as a decimal (e.g., 0.25 = 25%)
        """
...
def run_policy_simulation(
    ...
    **simulation_kwargs,
    ...
    Args:
        ...
        resource_amount: total budget to allocate at t=0
        cate_values: array of CATE estimates (len = n_agents), required for "cate_optimal"
        **simulation_kwargs: forwarded to run_simulation (e.g., n_periods, seed, etc.)
    """
    if not agents:
        raise ValueError("agents list cannot be empty")
...
def fit_bayesian_luck_model(
    ...
    Args:
        df: DataFrame from get_results_dataframe(agents)
        draws: number of posterior draws per chain
        tune: number of warmup/burn-in iterations
...
def summarize_bayesian_fit(idata, var_names: Optional[List[str]] = None) -> pd.DataFrame:
    ...
    Args:
        idata: ArviZ InferenceData returned by fit_bayesian_luck_model
        var_names: optional subset of parameter names to summarize
...
def posterior_predictive_check(
    ...
    Args:
        model: PyMC model returned by fit_bayesian_luck_model
        idata: ArviZ InferenceData with posterior draws
        df: same DataFrame used for fitting
        random_seed: RNG seed for reproducibility
