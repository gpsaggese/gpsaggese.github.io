...
def add_edge_features(
    ...
    # Convert timestamp to numeric (handles string timestamps)
    timestamps_numeric = pd.to_numeric(rclean["timestamp"], errors='coerce')
    timestamps = torch.tensor(timestamps_numeric.to_numpy(), dtype=torch.int64)
...
def add_movie_node_features(
    ...
    num_movie_nodes = g.num_nodes("movie")
    
    etype = ("user", "rates", "movie")
    if g.num_edges(etype) > 0:
        movie_node_indices = g.edges(etype=etype)[1].unique().cpu().numpy()
        reverse_map = {idx: raw_id for raw_id, idx in movie_id_map.items()}
        movies_in_graph = set(reverse_map[idx] for idx in movie_node_indices if idx in reverse_map)
    else:
        movies_in_graph = set()

    movies_df = movies_df.copy()
    movies_df["movieId"] = movies_df["movieId"].astype(int)
    movies_df = movies_df[movies_df["movieId"].isin(movies_in_graph)].copy()
    
    if len(movies_df) == 0:
        _LOG.warning("No movies from movies_df found in graph; skipping movie node features.")
        return
    ...
    filtered_movie_map = {raw_id: idx for raw_id, idx in movie_id_map.items() 
                          if raw_id in movies_in_graph}
    
 
    pairs = sorted(filtered_movie_map.items(), key=lambda kv: kv[1])  
    raw_ids_sorted = [int(raw) for raw, _ in pairs]
    
    final_df = features_df.reindex(raw_ids_sorted).fillna(0.0)
    
    if len(final_df) != num_movie_nodes:
        raise ValueError(
            f"Feature matrix shape mismatch: expected {num_movie_nodes} rows (graph nodes), "
            f"got {len(final_df)} rows. Movies in graph: {len(movies_in_graph)}, "
            f"Movies in map: {len(movie_id_map)}"
        )
...
def add_user_node_features(g: dgl.DGLHeteroGraph, embedding_dim: int = 32) -> nn.Embedding:
    ...
    g.nodes["user"].data["h"] = emb.weight  
...
def train_link_prediction(
    ...
    for ep in range(1, epochs + 1):
        ...
        neg_u, neg_v = negative_sampling_uniform(g, train_eids, num_neg=5)
...
def train_link_prediction_with_early_stopping(
    ...
    for ep in range(1, epochs + 1):
        ...
        neg_u, neg_v = negative_sampling_uniform(g, tr, num_neg=5)
        neg_u, neg_v = neg_u.to(dev), neg_v.to(dev)
        neg_scores = scorer(z_user[neg_u], z_movie[neg_v])
        neg_labels = torch.zeros_like(neg_scores)
...
def build_user_seen_map(
    ...
    if isinstance(g_or_df, dgl.DGLHeteroGraph):
        return build_user_seen_map_from_graph(g_or_df, eids_or_maps, etype=etype)  
    else:
        return build_user_seen_map_from_df(g_or_df, eids_or_maps)  
...
def load_genome_tags(raw_dir="data/raw/"):
    ...
    These files are part of the MovieLens 20M dataset and can be downloaded from:
    https://grouplens.org/datasets/movielens/20m/
    
    Files should be placed in raw_dir as:
    - genome_tags.csv (extracted from genome-tags.csv)
    - genome_scores.csv (extracted from genome-scores.csv)
    
    Parameters
    ----------
    raw_dir : str
        Directory containing genome_tags.csv and genome_scores.csv
    
    Returns
    -------
    tags_df : pd.DataFrame
        DataFrame with columns: tagId, tag
    scores_df : pd.DataFrame
        DataFrame with columns: movieId, tagId, relevance
    """
