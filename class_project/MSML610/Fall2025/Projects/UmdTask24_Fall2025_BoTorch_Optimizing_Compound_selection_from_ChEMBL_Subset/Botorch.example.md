...
flowchart TD
    ...
    A[Start] --> B[Import libraries and config]
    B --> C[Load raw ChEMBL data]
    C --> D[Clean and filter data]
    D --> E[Create descriptors and cost metric]
    E --> F[Split into train and test sets]
    F --> G[Train GP models]
    G --> H[Evaluate model performance]
    H --> I[Predict on candidate molecules]
    I --> J[Compute Pareto front]
    J --> K[Apply selection strategies]
    K --> L[Compare strategies to Pareto front]
    L --> M[Visualize results]
    M --> N[Export results and summaries]
    N --> O[End]
```
