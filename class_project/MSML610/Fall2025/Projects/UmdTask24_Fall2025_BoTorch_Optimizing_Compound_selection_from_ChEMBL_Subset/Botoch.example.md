...
flowchart TD
    Start([Start: Drug Discovery Project]) --> Load[Load ChEMBL AChE Data<br/>15,542 compounds]
    
    Load --> Filter[Data Filtering<br/>- IC50 measurements only<br/>- Valid SMILES<br/>- Remove duplicates]
    
    Filter --> Clean[Clean Dataset<br/>4,624 compounds<br/>29.8% retention]
    
    Clean --> Feat[Feature Engineering<br/>Calculate 13 Molecular Descriptors]
    
    Feat --> Desc[Descriptors Computed<br/>- Molecular Weight<br/>- LogP, HBD, HBA<br/>- Rotatable Bonds<br/>- TPSA, Rings<br/>- Heavy Atoms, etc.]
    
    Desc --> Cost[Compute Cost Metric<br/>cost = 0.5×MW/1000 + 0.5×SA/10]
    
    Cost --> Split[Train-Test Split<br/>70% Train: 3,237<br/>30% Test: 1,387]
    
    Split --> GP1[Train GP Model 1<br/>Target: pIC50 Potency]
    Split --> GP2[Train GP Model 2<br/>Target: Cost Metric]
    
    GP1 --> MLL1[Fit MLL<br/>Optimize Hyperparameters]
    GP2 --> MLL2[Fit MLL<br/>Optimize Hyperparameters]
    
    MLL1 --> Val1[Validate GP1<br/>R² = 0.88<br/>RMSE = 0.47]
    MLL2 --> Val2[Validate GP2<br/>R² = 0.99<br/>RMSE = 0.019]
    
    Val1 --> Pred[Multi-Objective Prediction<br/>Predict on Test Set]
    Val2 --> Pred
    
    Pred --> Pareto[Identify Pareto Front<br/>is_non_dominated function]
    
    Pareto --> ParetoResults[Pareto-Optimal Set<br/>57 compounds 4.1%<br/>Avg pIC50: 7.88<br/>Avg Cost: 0.42]
    
    ParetoResults --> Strat[Strategy Comparison<br/>5 Selection Methods]
    
    Strat --> S1[1. Random Selection]
    Strat --> S2[2. Top Potency]
    Strat --> S3[3. Lowest Cost]
    Strat --> S4[4. Balanced Ratio]
    Strat --> S5[5. Pareto-Optimal BoTorch]
    
    S1 --> Eval[Evaluate Strategies<br/>Trade-off Score = Potency/Cost]
    S2 --> Eval
    S3 --> Eval
    S4 --> Eval
    S5 --> Eval
    
    Eval --> Winner[Winner: Balanced Ratio<br/>Score: 39.46<br/>BoTorch 2nd: 33.61<br/>2.4× better than random]
    
    Winner --> Export[Export Results<br/>- Pareto compounds CSV<br/>- Strategy comparison<br/>- Visualizations]
    
    Export --> End([End: Optimized Compounds<br/>Ready for Synthesis])
    
    style Start fill:#e1f5e1
    style End fill:#e1f5e1
    style ParetoResults fill:#fff4e1
    style Winner fill:#ffe1e1
    style Val1 fill:#e1f0ff
    style Val2 fill:#e1f0ff
