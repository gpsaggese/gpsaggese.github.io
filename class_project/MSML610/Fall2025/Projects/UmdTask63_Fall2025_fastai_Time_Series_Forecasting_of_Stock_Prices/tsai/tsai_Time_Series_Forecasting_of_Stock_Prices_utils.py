"""Time Series Forecasting utilities powered by tsai.

This module mirrors the existing forecasting API but relies on the tsai
(Time Series AI) library for model creation and training.
"""
...
from tsai.all import InceptionTime, TSDataLoaders, TSDatasets, TSRegression, ts_learner

warnings.filterwarnings('ignore')

try:  # Optional dependency used for sentiment scoring
    import nltk
    from nltk.sentiment import SentimentIntensityAnalyzer
except ImportError:  # pragma: no cover - handles optional dependency missing
...
class StockDataCollector:
    ...
    def fetch_stock_data(symbol: str, start_date: str, end_date: str) -> StockData:
        try:
            ...
            raise Exception(f"Error fetching data for {symbol}: {exc}") from exc


class NewsSentimentAnalyzer:
    """Downloads relevant news and engineers sentiment signals."""
    ...
    def _init_sentiment_model(self):
        if SentimentIntensityAnalyzer is None:
            warnings.warn(
                "nltk is not installed; skipping news sentiment augmentation.",
                RuntimeWarning,
            )
            return None
        ...
        except Exception as exc:  # pragma: no cover - download or init failure
            warnings.warn(f"Unable to initialize VADER sentiment analyzer: {exc}")
            return None

    def _fetch_news(self, start_date: str, end_date: str) -> pd.DataFrame:
        if not self.api_key:
            warnings.warn(
                "No News API key supplied; news sentiment features disabled.",
                RuntimeWarning,
            )
            return pd.DataFrame()
...
class DataPreprocessor:
    ...
    def create_features(
        ...
        # Lagged variables capture richer temporal context
        for lag in [1, 2, 3, 5]:
...
class TimeSeriesForecaster:
    """Time series forecasting model using tsai architectures"""
    ...
    @staticmethod
    def _to_tsai_format(X: np.ndarray) -> np.ndarray:
        """tsai expects (samples, features, seq_len)."""
        return np.transpose(X, (0, 2, 1)).astype(np.float32)

    def prepare_data(self, X: np.ndarray, y: np.ndarray) -> TSDataLoaders:
        if len(X) < 2:
            raise ValueError("Not enough samples to create training/validation splits")

        self.feature_count = X.shape[2]
        X_tsai = self._to_tsai_format(X)
        y = y.astype(np.float32)

        split_idx = int(len(X_tsai) * self.config.train_split)
        split_idx = max(1, min(split_idx, len(X_tsai) - 1))
        splits = (list(range(split_idx)), list(range(split_idx, len(X_tsai))))

        tfms = [None, TSRegression()]
        dsets = TSDatasets(X_tsai, y, splits=splits, tfms=tfms)
    ...
    def train_model(self, X: np.ndarray, y: np.ndarray) -> None:
        ...
        self.learner.fit_one_cycle(self.config.epochs, self.config.learning_rate)
    ...
    def predict(self, X: np.ndarray, scaler: MinMaxScaler) -> np.ndarray:
        ...
        X_tsai = self._to_tsai_format(X)
        test_dl = self.learner.dls.test_dl(X_tsai)
        preds = self.learner.get_preds(dl=test_dl)[0].cpu().numpy().flatten()

        dummy_array = np.zeros((len(preds), len(scaler.feature_names_in_)))
        dummy_array[:, 0] = preds
    ...
    def evaluate_model(self, X_test: np.ndarray, y_test: np.ndarray, scaler: MinMaxScaler) -> Dict[str, float]:
        preds = self.predict(X_test, scaler)
        ...
        mae_value = mean_absolute_error(y_test_inverse, preds)
        with np.errstate(divide='ignore', invalid='ignore'):
            mape_values = np.abs((y_test_inverse - preds) / y_test_inverse)
...
def plot_predictions(actual: np.ndarray, predicted: np.ndarray, title: str = "Stock Price Predictions") -> None:
