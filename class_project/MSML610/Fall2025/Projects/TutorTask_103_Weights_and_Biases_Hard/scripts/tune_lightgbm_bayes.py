"""
Bayesian hyperparameter tuning for LightGBM using Optuna (TPE sampler).

Reads search space from:
  config/params.yaml -> hyperparameter_tuning.spaces.lightgbm

Writes:
  artifacts/tuning/lightgbm_trials.csv
  artifacts/tuning/lightgbm_best.json
"""
...
def main() -> None:
    ...
    space = (tuning_cfg.get("spaces", {}) or {}).get("lightgbm")
    if not isinstance(space, dict):
        raise ValueError("Missing hyperparameter_tuning.spaces.lightgbm in config/params.yaml")

    tuning_dir = Path(params.get("evaluation", {}).get("artifacts_dir", "artifacts")) / "tuning"
    tuning_dir.mkdir(parents=True, exist_ok=True)
    trials_csv = tuning_dir / "lightgbm_trials.csv"
    best_json = tuning_dir / "lightgbm_best.json"

    logger = WandbLogger("config")
    if not logger.run:
        logger.init_run(run_name="tune_lightgbm_bayes")
    ...
    study = optuna.create_study(direction=direction, sampler=sampler, study_name="lgbm_bayes")
    ...
    def objective(trial: "optuna.Trial") -> float:
        ...
        trainer.params["model"].setdefault("lightgbm", {})
        trainer.params["model"]["lightgbm"].update(hp)

        t0 = time.time()
        tr = trainer.train_lightgbm(data["X_train"], data["y_train"], data["X_val"], data["y_val"])
        ...
        if logger.run:
            logger.log_metrics({f"lgbm_tune/val_{k}": v for k, v in tr.metrics.items()})
            logger.log_metrics({"lgbm_tune/trial": trial.number, "lgbm_tune/elapsed_s": elapsed, f"lgbm_tune/val_{primary}": score})
    ...
    trainer.params.setdefault("model", {})
    trainer.params["model"].setdefault("lightgbm", {})
    trainer.params["model"]["lightgbm"].update(best_hp)
    best_tr = trainer.train_lightgbm(data["X_train"], data["y_train"], data["X_val"], data["y_val"])
    test_metrics = evaluator.evaluate(best_tr.model, data["X_test"], data["y_test"], "lightgbm_best")
    ...
    logger.info(f"Saved LightGBM tuning trials to {trials_csv}")
    logger.info(f"Saved LightGBM best config/metrics to {best_json}")
    if logger.run:
        logger.log_artifact(str(trials_csv), "lgbm_tuning_trials", "dataset")
        logger.log_artifact(str(best_json), "lgbm_tuning_best", "metadata")
