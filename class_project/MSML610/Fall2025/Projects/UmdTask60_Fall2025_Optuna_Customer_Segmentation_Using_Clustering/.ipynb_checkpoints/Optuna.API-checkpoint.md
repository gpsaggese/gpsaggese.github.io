...
## 6. Output Files (Generated in Notebook)

The notebook generates two CSV files after final model selection:

### cluster_summary.csv

**Purpose:** High-level cluster overview with business labels. One row per cluster.

**Columns:**
- `Cluster` (int): Cluster ID (0, 1, 2)
- `Customer_Count` (int): Number of customers in cluster
- `Avg_Recency` (float): Mean recency in days
- `Avg_Frequency` (float): Mean frequency (orders)
- `Avg_Monetary` (float): Mean monetary in GBP
- `Segment_Label` (str): Business-friendly name

**Example:**
```
Cluster | Customer_Count | Avg_Recency | Avg_Frequency | Avg_Monetary | Segment_Label
0       | 257            | 12.98       | 15.60         | 5858.91      | VIP Champions
1       | 1221           | 234.04      | 1.69          | 500.28       | At-Risk/Lost
2       | 2795           | 40.04       | 3.41          | 1199.83      | Loyal Customers
```

**Use:** Share with business stakeholders. Quick reference for segment profiles and naming.

---

### customer_segments_final.csv

**Purpose:** Customer-level segmentation with RFM scores. One row per customer.
...
- `Cluster` (int): Assigned cluster (0, 1, or 2)

**Example:**
```
CustomerID | Recency | Frequency | Monetary | Cluster
12346.0    | 326     | 1         | 104.00   | 1
12347.0    | 2       | 7         | 4127.05  | 2
12348.0    | 75      | 4         | 1302.08  | 2
...
```

**Use:** Export to CRM, email marketing platform, or analytics tool. Filter by cluster for targeted campaigns.
