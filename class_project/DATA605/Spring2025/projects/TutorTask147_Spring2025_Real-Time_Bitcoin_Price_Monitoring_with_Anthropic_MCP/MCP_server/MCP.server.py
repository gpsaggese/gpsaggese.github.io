...
async def detect_trend(days: int = 30) -> Dict[str, Any]:
    ...
    try:
        # Create a fresh copy of the market chart data to avoid reusing awaited coroutine
        ...
        # Clear the LRU cache for this specific days parameter to prevent coroutine reuse
...
async def plot_price(days: int = 7) -> str:
    ...
    try:
        ...
        logger.info(f"Price chart saved to {file_path}")
        
        # Get absolute file path to help users locate the file
        abs_file_path = os.path.abspath(file_path)
        logger.info(f"Full path: {abs_file_path}")
        
        # Clear the LRU cache to prevent coroutine reuse
        _fetch_market_chart.cache_clear() 
...
async def get_summary() -> Dict[str, Any]:
    ...
    try:
        ...
        # Get 24h price change with fresh data
