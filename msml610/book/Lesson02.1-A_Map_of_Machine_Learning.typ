...
A map of the field can orient before the details: it shows where a new method fits,
which tool suits a given problem, and what this course will and will not cover.
...
How do you set up a learning problem? The answer depends on what kind of feedback the
learner receives and how it interacts with its environment. @fig:learningparadigms
illustrates the major learning paradigms organized along two axes: the availability
...
In #strong[supervised learning], the learner receives a dataset of input-output pairs
and must discover a mapping from inputs to outputs that generalizes to unseen
examples. The labels are fully provided, making this the most straightforward
setting: given enough labeled data, the learner can directly measure how well its
predictions match the ground truth. In contrast, #strong[unsupervised learning]
...
leverages the structure of the unlabeled examples to improve its predictions beyond
what the few labeled points alone would support. A related but distinct paradigm is
#strong[self-supervised learning], in which the learner constructs its own
supervision signal from the data itself (for instance, predicting a masked word in a
sentence or the next frame in a video), effectively turning an unsupervised problem
into a supervised one without human annotation.
...
annotator) for the labels of specific examples it finds most informative, thereby
reducing the total labeling effort needed to reach a given performance level.
#strong[Reinforcement learning] goes further: the agent acts within an environment,
receives scalar reward signals rather than explicit labels, and must learn a policy
that maximizes cumulative reward over time. Here the feedback is not only partial but
delayed, since a single action's consequences may not become apparent until many
steps later. This sequential, interactive nature makes reinforcement learning
...
What functional form can a model take? Machine learning models generally fall into
...
#strong[Non-parametric models] make no such commitment. Instead of fixing the number
of parameters before training, they let model complexity grow with the size of the
dataset. k-Nearest Neighbors is a canonical example: at prediction time the model
consults the training set directly, so adding more data effectively adds more
"parameters." Kernel density estimators and Gaussian processes also belong here.
Non-parametric methods are more flexible and less prone to structural
misspecification, but they can be memory-intensive and slower at inference because
they must retain (or summarize) the training data.

#strong[Graphical models] take a different angle entirely. Rather than focusing on
whether the parameter count is fixed or flexible, they represent the joint
probability distribution over a set of variables as a graph, where nodes are random
variables and edges encode conditional dependencies. Bayesian networks (directed
acyclic graphs) and Markov random fields (undirected graphs) are the two main
sub-families. Graphical models excel at capturing complex multivariate structure,
reasoning under uncertainty, and making conditional independence assumptions explicit
and auditable.

These three families are not mutually exclusive. A Bayesian network, for instance,
can use parametric distributions at each node, or it can use non-parametric
conditional density estimators. The choice of family shapes everything downstream:
how training scales, what #emph[inductive biases] the learner brings, and how
transparent the resulting model is to inspection. @fig:modelfamilies summarizes this
taxonomy, showing how the three families branch from the central concept of an ML
model.
...
What are the #emph[stages] and #emph[common techniques] used in a machine learning
pipeline? A typical pipeline moves through several distinct phases, each with its own
set of tools and concerns.

The process begins with #strong[input processing], where raw data is transformed into
a form suitable for modeling. This includes data cleaning (handling missing values,
correcting errors, removing duplicates), dimensionality reduction (projecting
high-dimensional data into a lower-dimensional space to reduce noise and
computation), and feature engineering (constructing new input variables that better
capture the structure relevant to the prediction task).

Next comes #strong[model building], which involves two intertwined decisions:
choosing a functional form for the hypothesis (a linear model, a decision tree, a
neural network, etc.) and selecting a learning algorithm that fits the parameters of
that model to the training data. The choice of model family constrains what kinds of
patterns the system can represent, while the learning algorithm determines how
efficiently and reliably it finds a good fit within that family.

Once a model is trained, #strong[performance evaluation] measures how well it
generalizes. Cross-validation is the standard technique here: the data is split into
complementary subsets so the model is always evaluated on examples it was not trained
...
effective degrees of freedom to improve generalization.

Finally, #strong[aggregation] methods combine multiple models to achieve better
performance than any single one. Boosting #cite("freundschapire1997boosting") trains
a sequence of weak learners, each focusing on the mistakes of its predecessors.
Bagging #cite("breiman1996bagging") trains multiple models on bootstrap samples of
the data and averages their predictions to reduce variance. Stacking learns a
meta-model that optimally combines the outputs of several diverse base learners.
...
What is the theoretical basis underpinning machine learning and its common phenomena?
Several foundational frameworks address this question, each offering a different lens
on why learning from data works and when it fails.

#strong[VC theory] #cite("vapnikchervonenkis1971uniform") measures a model's
#emph["capacity"] by examining the complexity of its hypothesis space. Rather than
counting parameters directly, VC dimension quantifies how many data points a model
class can shatter (classify in every possible way), providing distribution-free
guarantees on generalization. A model with high VC dimension can fit more patterns
but also needs more data to generalize reliably.
...
to data generated by a quadratic process. #emph[Variance] captures sensitivity to
fluctuations in the training set: a highly flexible model may fit one sample well but
produce very different predictions on another. The third term, #emph[irreducible
  noise] ($sigma^2$), reflects inherent randomness in the data that no model, however
powerful, can eliminate. The practical consequence is a tradeoff: reducing bias (by
using a more flexible model) typically increases variance, and vice versa.

The #strong[description length] framework #cite("rissanen1978mdl") connects learning
to information theory and compression. The #emph[Minimum Description Length] (MDL)
principle judges a model by the total cost of describing both the model itself and
the data given that model, measured in bits. A model that is too simple requires many
bits to encode the residual errors; one that is too complex requires many bits to
encode the model's own parameters. The best model strikes a balance, compressing the
overall description most efficiently.

The #strong[Bayesian approach] #cite("bishop2006prml") treats machine learning as
probabilistic inference. Instead of selecting a single best hypothesis, it maintains
a distribution over hypotheses, combining prior knowledge with observed data through
Bayes' theorem to update beliefs. This framework naturally incorporates uncertainty:
predictions reflect not just the most likely model but the entire posterior
distribution, and model complexity is penalized implicitly through the marginal
likelihood.
...
coherent probabilistic calculus. That said, a common difficulty in ML theory is that
the assumptions each framework requires may not align neatly with the messy,
...
chosen for computational convenience rather than genuine belief. Recognizing these
...
also the most plausible, a principle formalized in #emph[Bayesian] model selection
and minimum description length.

Other adages focus on data quality and quantity. #emph[Garbage in, garbage out]
(Fuechse, 1957) reminds us that no algorithm can rescue fundamentally flawed inputs.
George Box's observation that #emph[all models are wrong, but some are useful] #cite(
  "box1976science",
) reframes the goal: we are not seeking truth in a model's assumptions, only
predictive or explanatory value. On the darker side, Ronald Coase warned in 1982 that
#emph[if you torture the data long enough it will confess whatever you want], a
caution against p-hacking, overfitting, and confirmation bias in analysis.

More recent adages reflect the data-driven era. Clive Humby's 2006 phrase #emph[data
  is the new oil] cast data as a raw resource that, like petroleum, must be refined
to unlock its value. Peter Norvig's complementary claim that #emph[more data beats
  clever algorithms] (2006) was later expanded into the influential paper on
#emph[the unreasonable effectiveness of data] #cite(
  "halevy2009unreasonable",
), which argued that simple models trained on massive corpora routinely outperform
sophisticated models trained on small datasets. Finally, Rich Sutton's #emph[bitter
  lesson] #cite("sutton2019bitterlesson") sharpened this point further: across the
history of AI, general methods that leverage computation (search and learning at
scale) have consistently won out over systems that try to encode human domain
knowledge by hand. The lesson is "bitter" because researchers repeatedly invest years
in clever, knowledge-intensive approaches only to be overtaken by brute-force
scaling.

Taken together, these adages trace a shift in emphasis: from model elegance and
careful feature engineering toward large-scale data and computation as the dominant
drivers of progress.
...
computation and scale outperform hand-crafted, domain-specific approaches. These
principles reflect a fundamental shift in the field: from elegant models and careful
