...
high-dimensional parameter spaces encountered in modern models.

@fig:gradientdescent2 illustrates how gradient descent iteratively moves toward a
minimum of the objective surface.
...
direction, then take a step along the #emph[steepest downhill direction]. You repeat
this process until you arrive at a point where no further descent is possible. This
intuitive procedure is precisely what gradient descent formalizes for mathematical
optimization, as @fig:gradientdescent1 illustrates.
...
#strong[Gradient descent] is a general technique for minimizing twice-differentiable
...
functions are convex include logistic regression and linear regression, which is one
reason these models are so widely used in practice: their optimization landscape
contains no misleading valleys or plateaus.
...
$hat(bold(v))$ aligns with the gradient. The scalar product $Delta E_(i n)$ reaches
its most negative value when $hat(bold(v))$ points exactly opposite to the gradient,
that is, when
...
Each iteration therefore moves the weight vector a fixed distance $eta$ in whichever
direction reduces the error most steeply. The method is called #strong[gradient
  descent] precisely because it descends along the gradient of the objective
function, following the locally steepest downhill path at every step.
...
Each weight moves in the direction that locally reduces the in-sample error, scaled
by the learning rate $eta$ and normalized by the gradient's magnitude. A critical
detail is that the update of all components must be #emph[simultaneous]: every
partial derivative is evaluated at the current point $bold(w)(t)$ before any
component is changed, so no single coordinate's new value contaminates the
computation of another's update within the same step.

An #strong[iteration] of gradient descent refers to one complete application of this
update rule, producing a new weight vector from the current one. Counting iterations
gives a natural measure of computational effort and is the basis for convergence
...
however, this exact condition may never be satisfied numerically due to finite
floating-point precision and the iterative nature of the updates. Two practical
stopping criteria are commonly used instead:

- #emph[Threshold on variation]: stop when the change in $E_("in")$ falls below a
  small threshold, $Delta E_("in") < theta$.
...
$J(bold(w))$ to determine each update step. In practice, it is wise to periodically
recompute the cost function $J(bold(w))$ itself and verify that it is actually
decreasing. If the cost plateaus or begins to increase, that signals a problem: the
learning rate may be too large, causing the iterates to overshoot, or the algorithm
may have reached a region of the loss surface where the gradient provides poor
guidance. Periodically logging $J(bold(w))$ provides a simple but effective
diagnostic for catching these issues early.
...
When $eta$ is too small, the linear approximation of $E_(i n)$ remains effective at
each step, so every update does move downhill, but the steps are tiny and convergence
to the minimum requires a large number of iterations. @fig:gradientdescent3a
illustrates this slow, cautious trajectory.
...
When $eta$ is too large, the linear approximation breaks down because higher-order
terms in the Taylor expansion dominate the actual function values. Instead of
descending smoothly, the iterates overshoot the minimum and bounce back and forth
across the valley, potentially diverging altogether. @fig:gradientdescent3b shows
...
gradually reducing $eta$ as a function of the iteration count allows it to settle
into a tighter neighborhood of the minimum. Smaller learning rates in later stages
can even help the optimizer find a better local minimum in non-convex landscapes. The
tradeoff is that a #strong[learning-rate schedule] introduces an additional
hyperparameter (or family of hyperparameters, such as a decay rate and a decay
schedule) that must itself be tuned, adding complexity to the training pipeline.
@fig:gradientdescent3c depicts the effect of a well-chosen adaptive schedule, where
early large steps give way to finer adjustments near the optimum.
...
update rule normalizes the gradient so that only its direction matters:

$ Delta bold(w) = - eta frac(nabla J, \|nabla J\|) $

This keeps the step size constant at $eta$, but that uniformity creates a problem. On
steep regions of the loss surface, a fixed step may be too small, making progress
painfully slow. Near a minimum, where the surface flattens out, that same fixed step
may be too large, causing the optimizer to overshoot and bounce back and forth across
the valley instead of settling in.

To converge quickly, the learning rate should adapt to the local geometry. The
optimizer should take large steps when the gradient is large (the surface is steep
and far from a minimum) and small steps when the gradient is small (the surface is
nearly flat and close to a minimum). In other words, the ideal learning rate is
proportional to the gradient magnitude: $eta prop \|nabla J\|$.
...
Because the gradient vector $nabla J$ is not divided by its own norm, its magnitude
naturally scales the step size. When the surface is steep, $\|nabla J\|$ is large and
the update moves farther; when the surface is gentle, $\|nabla J\|$ is small and the
update shrinks. The scalar $eta$ still controls the overall scale, but the gradient's
magnitude now acts as a built-in adaptive factor. This simple change gives the
optimizer the variable-rate behavior needed for efficient convergence: aggressive
movement far from the optimum and cautious refinement near it.
...
elongated ellipses rather than circles. A single learning rate cannot serve both
directions well: it is too large along the steep, short-axis direction (causing the
updates to overshoot and bounce) and too small along the flat, long-axis direction
(causing the optimizer to crawl). The result is slow, unstable convergence that
zigzags across the narrow valley instead of heading straight for the minimum.

The fix is to rescale every feature so that all of them span comparable ranges before
training begins. Two common choices are #emph[min-max scaling], which maps each
feature to the interval $[0, 1]$, and #emph[standardization], which centers each
feature at zero with unit variance. Either approach reshapes the elongated ellipses
into something much closer to circles, so a single learning rate works reasonably
well in every direction and gradient descent converges faster and more smoothly.
...
observed targets across the entire dataset. Minimizing it drives the model toward a
set of weights that fits the training data well.

#strong[Batch Gradient Descent (BGD)] updates the weight vector by stepping in the
direction opposite to the normalized gradient of the in-sample error:
...
The inner sum sweeps over every training example, accumulating the product of the
current residual and the local sensitivity of the hypothesis to $w_j$. Only after
that full pass does the algorithm take a single step.

That full-dataset requirement is the chief drawback of batch gradient descent. When
$N$ is large (say, $N = 10^6$), every single weight update demands a gradient
evaluation that touches all $10^6$ examples, making each iteration computationally
expensive. The entire dataset must also reside in memory at once, which can be
prohibitive for large-scale problems. These costs motivate the stochastic and
mini-batch variants discussed next, which trade a noisier gradient estimate for
dramatically cheaper per-step computation.
...
gradient descent but instead oscillates, sometimes moving away from the optimum
before correcting course. Near a local minimum the updates do not settle cleanly;
they bounce around the basin rather than converging to a fixed point.
...
In other words, although any single SGD step may point in a direction that differs
from the true gradient, on average the steps point the same way as the batch
gradient. Over many iterations the random deviations cancel out, and the algorithm
makes steady progress toward a minimum of the in-sample error. The oscillation that
is a drawback in the short term actually confers a benefit: the noise helps SGD
escape shallow local minima that batch descent might get trapped in, often leading to
...
gradient over the entire training set (as in batch gradient descent) or from a single
example (as in stochastic gradient descent), mini-batch gradient descent uses a
subset of $b$ examples to compute each update to the current weight vector. Here $b$
is called the #emph[batch size], and common choices are $b = 32$ or $b = 64$. This
middle ground reduces the variance of parameter updates compared to the single-sample
stochastic approach, while remaining far more computationally tractable per step than
a full-batch computation. Mini-batch updates also benefit from hardware-level
parallelism on modern GPUs, which are optimized for matrix operations on moderately
sized batches. The choice of batch size introduces its own tradeoff: smaller batches
inject more noise into the optimization trajectory (which can help escape shallow
local minima), while larger batches yield smoother, more stable gradient estimates at
...
dataset is collected up front and the model is trained once.

Real-time systems make this requirement especially pressing. A deployed model needs
to adapt to new data points without full retraining, and it must handle variation in
the underlying process dynamics: the distribution generating the data may shift over
time, so yesterday's optimal parameters may no longer be adequate today.
...
illustrates. Because these methods update the model one example (or one small batch)
at a time, they fit directly into a #emph[streaming pipeline]: each incoming
observation triggers a parameter update, after which the raw data point can be
discarded. The model itself serves as a "compressed" representation of everything
seen so far, which is valuable when storing every data point is impractical. Consider
stock market prediction, where tick-by-tick price data accumulates at enormous
volume, or training a language model on live chat data, where older conversations can
be dropped after the model has absorbed their signal.

That said, discarding the data is generally #emph[a bad idea] whenever it can be
avoided. Once raw examples are gone, there is no way to revisit them if the learning
rate was poorly chosen, if the model class changes, or if a subtle bug corrupted
...
paradigm, a big data framework that splits distributed computation into two phases.
In the #emph[map step], $k$ worker machines each compute partial gradient sums over
their local shard of the data. In the #emph[reduce step], those $k$ partial sums are
sent to a single node that accumulates them into the full gradient, which is then
used to update the model parameters. As @fig:mapreduceforbatchgradientdescent
illustrates, the shuffle-and-sort stage routes each worker's partial gradient to the
aggregation node, which produces the aggregated gradient and applies it to the
current model parameters. Many learning algorithms beyond plain gradient descent
share this sum-then-update structure, so map-reduce serves as a general-purpose
backbone for large-scale model training whenever the objective decomposes into
independent per-example terms.
