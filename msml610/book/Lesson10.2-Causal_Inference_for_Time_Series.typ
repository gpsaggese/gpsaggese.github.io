...
By contrast, #strong[cross-sectional causal inference] compares different units at a
single point in time---for instance, comparing treatment outcomes between patients in
one cohort. Cross-sectional designs assume units are (conditionally) exchangeable,
which is straightforward but requires many comparison units.

Time series designs exploit what we might call the #strong[arrow of time]: causes
must precede effects. This principle eliminates backward arrows from causal graphs a
priori, pruning half the possible causal directions. However, time series designs
face distinct challenges. Observations are correlated across time, treatments and
confounders may drift or trend, and experiments are often impossible. A canonical
example illustrates the difficulty: did the Federal Reserve rate cut in 2020 cause
...
structure in a #strong[temporal causal graph]. @fig:temporal-causal illustrates a
simple first-order graph showing how variables at consecutive time points relate. The
restriction that $T_t$ affects $Y_t$ but not prior outcomes is encoded by the arrow
structure.
...
Consider a simple example: regressing daily stock returns on a dummy variable for
"news day." Stock returns exhibit strong autocorrelation. If we ignore this and apply
standard OLS, we compute incorrect standard errors, likely concluding that news has a
statistically significant effect when it does not.

The solution is to use #strong[Newey-West or HAC] (heteroskedasticity- and
autocorrelation-consistent) standard errors. These estimators account for correlation
at multiple lags and asymptotically produce correct inference. When autocorrelation
...
entirely unrelated series that happen to trend upward will appear highly correlated
even though one does not cause the other. The example of pirates and global
temperature is famous: the number of pirates has declined over two centuries while
global temperature has increased, creating a strong negative correlation---yet
...
stimulate the economy, which increases inflation. Policymakers then raise rates to
combat inflation, which dampens growth. The policy ($T_t$) affects inflation ($Y_t$),
but inflation simultaneously influences future policy. This reciprocal relationship
invalidates naive causal inference, as illustrated in @fig:feedback-loop.

Another example is advertising and sales. Companies increase advertising budgets when
sales rise (or to recover from low sales). Sales rise partly due to advertising and
partly due to external demand shocks. Advertising in turn responds to sales, creating
bidirectional causality that OLS cannot untangle.
...
loops. Rather than treating policy and outcome asymmetrically (as in observational
causal inference), VAR models treat all variables symmetrically, estimating how they
...
(uncorrelated across equations). The VAR estimates how shocks propagate through the
...
variation in treatment. The key idea is finding a variable $Z$ that affects treatment
$T$ but not outcome $Y$ directly (the #strong[exclusion restriction]). We use $Z$ to
predict $T$, obtaining , then measure how correlates with $Y$.

This approach works because $Z$ provides variation in treatment independent of
confounds, allowing us to isolate the causal effect. In education, scholars study the
effect of schooling on earnings using distance to the nearest college as an
instrument: distance affects educational access but not earnings directly (barring
unusual circumstances).

The limitation is that IV estimates effects only on #strong[compliers]---units whose
treatment changes with the instrument. Compliers may differ from the full population,
...
#strong[Natural experiments] exploit unexpected external events that randomly assign
treatment to some units but not others, mimicking a randomized trial in observational
data. The advantage is that randomization eliminates confounding without requiring an
instrument.

A famous example is the Vietnam War draft lottery, which randomly assigned military
service. Researchers compared drafted and non-drafted veterans on lifetime earnings,
estimating the causal effect of military service. Because the lottery was random, we
can attribute earnings differences to military service, not selection effects.
...
Consider evaluating a diet on weight loss. Season (winter vs. summer), stress, and
motivation all change over time and drive both adherence and weight change. These are
hard to measure but crucial confounders. Naive before-after comparisons will be
...
#strong[Spillovers and SUTVA violations] happen when treatment on one unit affects
another. If advertising in one region spills to neighboring regions, we cannot
isolate the effect to the treated region.

#strong[Structural breaks] occur when the data-generating process changes mid-sample.
The 2008 financial crisis and COVID-19 pandemic are extreme examples, but smaller
breaks (policy regime changes, technological shifts) are common. Models fit before
the break may not hold afterward.

#strong[Missing data and irregular sampling] arise because standard time series tools
assume equispaced observations. Sensor dropouts, holidays, weekends, and other
...
Temporal structure is a resource for causal inference when exploited correctly. Time
...
Natural experiments are easier to spot in time series. Policy changes, shocks, and
discontinuities provide quasi-experimental variation. Interrupted Time Series,
Difference-in-Differences, and synthetic control all exploit this variation. When
studying the effect of a sugary-drink ban, for example, we compare the same city
...
Seasonality hides effects when a seasonal bump masks or amplifies a policy effect.
...
inflates correlations: if $Y_t$ drives and $T_t$ drives $Y_t$, regressing $Y_t$ on
$T_t$ picks up both the treatment effect and the reverse effect, confounding
inference.

Regime changes invalidate models. A model fit before COVID may not generalize after.
...
It is important to understand what Granger causality is NOT. It is not causation in
...
Granger causality relies on several assumptions, violations of which lead to spurious
conclusions. #strong[Stationarity] is essential: both series must have time-invariant
statistical properties. Non-stationary series can produce spurious Granger causality.
The remedy is to pre-test with unit-root tests (Augmented Dickey-Fuller, KPSS) and
difference series if needed.

#strong[Correct lag order $p$] is critical. Too few lags miss the true effect, while
too many lead to overfitting and loss of power. Lag order is typically chosen via
information criteria (AIC, BIC) or cross-validation. Under- or over-specification
biases tests.

#strong[Linearity] is assumed in the basic formulation. Non-linear Granger causality
extensions exist (e.g., transfer entropy and machine-learning based methods) but are
beyond standard practice.

#strong[No omitted common causes]---this is the "big one." If an unobserved variable
$Z$ drives both $X$ and $Y$ with different lags, $X$ will appear to Granger-cause $Y$
even though it does not. This cannot be tested; we must rely on domain knowledge.

#strong[Correct sampling frequency] is needed. If the true effect operates faster
...
may miss daily Granger effects. If the process evolves faster than the sampling rate,
we observe spurious bidirectional causality (each series Granger-causing the other)
...
exist (VAR formulations), but standard Granger causality tests skip contemporaneous
relationships.
...
weekly data on ad spend $X_t$ and sales $Y_t$. We fit both the restricted model
(sales on past sales only) and unrestricted model (sales on past sales and past ads).
...
stock returns. Research finds mixed results: sentiment does Granger-cause
...
#strong[Interrupted Time Series] (ITS) designs are used when a policy or intervention
is implemented at a single known date t_start across all (or most) units. Examples
include a new speed limit enforced nationwide from January 1st, or a website redesign
launched on a specific day.
...
ITS has impressive real-world examples. Seatbelt laws reduced traffic fatalities by
approximately 15--20%. Netflix's password-sharing ban had mixed results: growth in
some regions but churn in others, illustrating that counterfactuals are often
uncertain. Apple's iOS privacy changes (requiring apps to request tracking
permission) saw opt-in rates plummet from 60--70% to 15--25%, devastating ad
targeting. Meta reported billions in revenue losses, demonstrating the power of
identifying when policy changes affect outcomes.

As illustrated in @fig:its-structure, the core ITS strategy is to extrapolate the
pre-period trend as a counterfactual, then compare it to the actual post-period
outcome. The gap represents the policy effect.
...
Always check for confounding events near t_start : seasonality, holidays, economic
...
group (pre to post) and subtract the change in the control group. The residual is the
estimated causal effect, with the visual arrangement highlighting how both
between-group differences and common time trends are removed.
...
#strong[Visual check]: plot the two group means over time. Do they move in lockstep
before treatment? Divergence suggests violation.

#strong[Placebo / event-study regression] is more formal:

We estimate effects at all lags and leads relative to treatment. For pre-treatment
periods, coefficients should be near zero (no pre-treatment effect). For
post-treatment periods, we capture dynamic treatment effects.
...
#strong[OLS with fixed effects] gives the same point estimate as simple group means
but allows covariates. Covariates help when they affect outcomes but are balanced
across groups. They reduce variance without introducing bias.

#strong[Balanced vs. unbalanced panels]: missing data or attrition biases estimates
if non-random. Always report attrition rates and check whether balance is maintained
across groups and time.

#strong[Common mistakes]:
- Forgetting that treatment and time indicators enter as main effects, not just the
  interaction.
...
#strong[Alternative control groups]: use different comparison units. Results should
be stable across reasonable choices. If the effect flips when you switch controls,
...
In reality, units are often treated at #strong[different times] (staggered rollout).
States adopt laws across different years; companies roll out products sequentially to
different regions. Traditional #strong[two-way fixed effects] (TWFE) models all these
with:
...
- #strong[Callaway & Sant'Anna (2021)]: estimate group-time average treatment
  effects, ATT(g, t), for each cohort g at time t, then aggregate with clean weights.
- #strong[Sun & Abraham (2021)]: interaction-weighted event-study estimator avoiding
  negative weights.
- #strong[de Chaisemartin & D'Haultfœuille (2020)]: robust estimators under effect
  heterogeneity.
...
Suppose we have J+1 units: unit 1 is treated at time t_start , units 2, ..., J+1 are
untreated (donors). Let X_1 be a vector of pre-treatment predictors for the treated
unit, and X_0 is a matrix of the same predictors for donors (size k by J matrix).
...
outcomes. This is a nested optimization: the inner loop finds weights for a given
predictor matrix; the outer loop finds $V$.
...
donor with poor pre-fit produces noise, not a legitimate null. In the California
...
#strong[Long pre-treatment period]: many time points allow matching on pre-treatment
characteristics. Rule of thumb: at least 10+ observations.

#strong[Rich donor pool]: many untreated units with diverse characteristics enable
good pre-treatment fit via convex combinations.

#strong[Small number of treated units], usually one. Synthetic control is designed
for aggregated units: countries, states, cities, or firms.

#strong[Outcome with moderate noise], low enough to detect signal above background
variation.
...
affected by a policy, they are not valid controls. This violates the exclusion
assumption.

#strong[Short pre-period or noisy outcome] makes fit unreliable or prone to
overfitting noise.

#strong[Confounding unobserved events] near t_start masquerade as treatment effects.
If California's economy experienced a shock around the tobacco law, synthetic control
attributes it to the law.

#strong[Interpolation bias] occurs when covariates are imbalanced: synthetic control
averages over donors who differ in important but unmeasured ways.
...
#strong[Use Granger causality] when you want a fast predictive screen on two long
time series and no intervention/experiment is available.

#strong[Use ITS] when a single intervention has a sharp date and no comparison units
exist, with a long and stable pre-period.

#strong[Use DiD] when some units are treated and others are not, parallel pre-trends
are plausible, and you have panel data with units and time.
