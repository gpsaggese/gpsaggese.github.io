...
Over the past decade, the dominant paradigm in data-driven organizations has been to
collect and organize massive volumes of data, then run analytics on that data in the
form of dashboards, statistical models, and reports. This approach has produced
tremendous value, but it also has fundamental limitations that become apparent when
organizations attempt to move from understanding the past to shaping the future.

#strong[Data analytics] encompasses several categories of increasing sophistication.
At the most basic level, organizations maintain #strong[collections of
  data]---organized datasets such as customer purchase histories stored in CRM
systems. #strong[Descriptive statistics] summarize these datasets using key metrics
like mean, median, and standard deviation, enabling teams to track trends such as
average quarterly sales. #strong[Historical reports] review past performance, for
instance monthly sales reports from the previous year. #strong[Dashboards] provide
visual summaries of key metrics for quick insights, such as quarterly revenue and
expense overviews. Finally, #strong[models] use statistical tools to forecast or
explain phenomena, such as predicting customer churn from behavioral data.
...
#strong[Descriptive analytics] answers "What happened?" through summary statistics
and historical reports. #strong[Predictive analytics] answers "What will happen?"
...
Traditional machine learning systems create what we might call an #strong[illusion of
  understanding]. These systems excel at finding patterns in historical data and
making accurate predictions, but they fundamentally cannot explain the mechanisms
behind those patterns or answer "what if" scenarios.
...
This creates a real problem for decision-makers: without understanding causation,
organizations can only react to past events. They cannot proactively change outcomes
because they lack the knowledge of which interventions will produce desired effects.

// Slide: Association, Correlation, and Causation

Understanding the distinctions between #strong[association], #strong[correlation],
and #strong[causation] is fundamental to appreciating why traditional ML falls short.

#strong[Association] means that variables co-occur or are statistically dependent.
For example, "people who buy diapers also buy beer." Association provides no
information about direction or mechanism---it simply indicates that knowing one
variable tells you something about the other.
...
Machine learning systems provide genuine value in several domains. #strong[Pattern
  recognition] allows models to find correlations in data, identify similar items
through clustering and classification, and forecast based on past trends. ML delivers
#strong[predictive accuracy on held-out test data], meaning it can make accurate
predictions as long as conditions remain stable---that is, when training data and
future data share the same distribution. ML also identifies #strong[feature
  importance], revealing which variables have the strongest statistical associations
with outcomes. However, feature importance fundamentally cannot distinguish
...
understanding, intelligent decision-making is impossible.

// Slide: Correlation is Not Causation: Examples

Intuitively, most people understand that association is not causation, yet humans
routinely confuse the two. Common examples include inferring that food $X$ is bad for
you because eating it preceded a stomachache, or believing you can time markets
because you happened to buy stocks before price spikes.

Sometimes both causal directions are plausible: does top-notch consulting improve
...
pharmaceuticals) and timing (before Christmas versus mid-summer).

// Slide: Problem 2: Decision Making

Businesses need #strong[decisions, not just predictions]. Consider the question:
"What happens if a product price is reduced by 10%?" This seemingly simple question
spawns many causal sub-questions: Will more customers buy? Will revenue decrease, and
if so, what action should follow? Why are customers leaving---is it a quality issue
or an emerging competitor?

Examples of policy questions that require causal reasoning include: "Low-sugar versus
low-fat diet: which works for weight loss?", "If we increase credit lines, how does
bank margin change?", and "Should we build a library or give tablets to boost test
scores?" None of these can be answered by prediction alone.

// Slide: Problem 3: ML Explainability

Regulators increasingly require organizations to defend their ML-based decisions,
particularly in sensitive domains like hiring and insurance policy setup. Many ML
systems---especially neural networks---function as #strong[black boxes] where humans
cannot understand the input-to-output process. This creates serious problems:
organizations cannot explain decisions to shareholders, and features like age, race,
and sex may introduce bias that is invisible within the model.

The consequences of unexplainable AI include regulatory fines, customer backlash, and
activist pressure. The solution lies in #strong[explainable AI] that lets users
comprehend, explain, and trust machine-generated results. Causal models are
inherently more interpretable because they represent the mechanisms by which
variables influence each other.

// Slide: Problem 4: How ML Systems Make Decisions Today

The typical ML workflow follows a simple pipeline: collect data, train a model, make
predictions, and deploy with rules like "If model predicts $X$, take action $Y$." The
critical limitation is that these systems optimize for prediction accuracy rather
than decision quality or business outcomes. A fraud detector with 95% accuracy that
blocks 50% of legitimate transactions is technically accurate but practically
catastrophic for the business.

// Slide: Data Science for Decisions: Examples

Consider three common applications where optimizing for prediction accuracy misses
the mark:

#strong[Recommendation systems] (Netflix, Amazon, TikTok) predict which items users
will click based on historical behavior, then recommend top-$N$ items. The problem is
that recommending novel items has no historical data to learn from, and optimizing
purely for clicks may harm long-term engagement through boredom, echo chambers, and
filter bubbles.

#strong[Hiring pipelines] score candidates and rank applicants, but training data
encodes historical hiring bias. The model perpetuates past discrimination rather than
identifying the best candidates.

#strong[Fraud detection] systems flag suspicious transactions and block them
automatically, but high false positive rates persist without understanding the causal
mechanisms behind fraudulent versus legitimate behavior.

// Slide: Problem 5: Feedback Loops

Prediction systems can change the world through #strong[feedback loops], creating
self-reinforcing cycles that amplify initial biases.

In #strong[recommendation systems], when a model recommends sensational content and
users click more, future training data contains more clicks on sensational content,
causing the model to recommend even more extreme material. This leads to
radicalization, echo chambers, and mental health issues.

In #strong[criminal justice relapse prediction], minorities are policed more heavily,
leading to more arrests, which biases the training data. Models trained on arrest
data (biased against minorities) are then used by judges for bail and sentencing
decisions, creating an increasingly biased system over time.

In #strong[college admissions], models trained on past acceptance decisions encode
historical racism and sexism. When used to screen applications, these biases are
perpetuated and potentially amplified through each admission cycle.
...
#strong[Concept drift] occurs when $Pr(Y | X)$---the relationship between inputs and
predictions---changes over time. For example, customer preferences evolve, so a model
trained on 2020 data becomes less accurate by
2023.

#strong[Covariate shift] occurs when $Pr(X)$---the input distribution---changes while
the underlying relationship remains stable. For example, if more young customers join
a platform, a model trained on an older customer base performs worse on the new
population.

#strong[Label shift] occurs when $Pr(Y)$---the outcome distribution---changes. For
...
#strong[Optimization] finds the best action given a *known* objective. It assumes the
world's behavior is understood and seeks to minimize cost, maximize throughput, or
tune parameters. Tools include gradient descent, linear programming, and evolutionary
algorithms.

#strong[Statistical inference] estimates parameters from data. It learns from
observations given a model structure---for example, estimating treatment effects or
computing confidence intervals. Tools include regression, Bayesian estimation, and
hypothesis testing.

#strong[Decision theory] chooses actions under *uncertainty*. Questions like "Should
we launch a product with uncertain demand?" require reasoning about interventions and
their uncertain outcomes. Tools include utility functions, expected value of
information, and causal models.
...
When organizations ignore causality, they make wrong decisions and waste resources.
Several well-known statistical phenomena illustrate this cost:

#strong[Simpson's Paradox] occurs when aggregate data shows the opposite trend from
subgroups. The classic example is Berkeley admissions: overall data suggested gender
bias, but within individual departments no bias existed. The causal fix is to
condition on the right variables using a directed acyclic graph (DAG).

#strong[Confounding bias] arises when a hidden variable drives both the treatment and
the outcome. Ice cream sales and drowning deaths both rise in summer---not because
ice cream causes drowning, but because warm weather (the confounder) causes both. The
causal fix is to identify and control for confounders.
...
#strong[Feedback loop amplification] occurs when biased predictions reinforce bias in
future training data. More police sent to minority neighborhoods leads to more
arrests, which makes the model learn those neighborhoods are more dangerous, which
sends more police. The causal fix is to model feedback explicitly within the system.
...
confounded by patient severity, since sicker patients require more staff. A naive
intervention ("reduce staff to lower mortality") would be catastrophic. The causal
fix is to distinguish correlation from causal effect before taking action.
...
#strong[Current AI] uses correlation to analyze data, identify patterns, and make
predictions. Model quality depends entirely on data quality---biased or unclean data
...
#strong[Rung 1: Association] addresses the question: "How would seeing $X$ change
your belief in $Y$?"

Mathematically represented as $Pr(Y|X)$, this rung involves passive observation to
determine whether $X$ and $Y$ are related. Traditional AI and ML operate entirely at
this level. Examples include observing that "trees have green leaves in spring,"
asking "what does a symptom tell you about a disease?" or "what does a survey tell
...
causal model, not just observations. The `do()` operator represents an active
intervention that breaks the natural data-generating process.

Examples include: "Spring makes leaves turn green" (a causal mechanism), "Did the
headache disappear from the pain reliever or from food?", "If I take aspirin, will my
headache be cured?", and "If we ban sugary sodas, what happens to obesity rates?"
...
dose?", "What would the jury have concluded with different evidence?", and "Why did
...
Both correlation-based and causation-based models accept inputs, transform data, and
compute predictions. However, they differ fundamentally in their approach and
capabilities.

#strong[Correlation-based AI] works best with abundant historical data. It identifies
statistical patterns and uses them for prediction. This approach is powerful when the
goal is forecasting under stable conditions.

#strong[Causal-based AI] requires building a business model first, then integrating
data. It identifies mechanisms and enables reasoning about interventions. This
approach is necessary when the goal is decision-making or understanding effects of
actions.

// Slide: Correlation-Based Model Process

The #strong[correlation-based model process] follows a "data first" approach where
more data is assumed to be better. The typical process involves: acquiring data,
integrating and cleaning it, performing exploratory data analysis, engineering
features, building and testing models, and deploying models in production.

Common failures of this approach include organizational issues (teams working in
silos), opaque models that lack explainability, spurious correlations that mislead
decision-makers, and a failure to ever articulate "what is the business goal?" The
data-first approach often produces technically impressive models that solve the wrong
problem.
...
begins with fundamental questions: What is the intended outcome? What is the proposed
intervention? What are the confounding and effecting factors? Only after answering
...
and decisions involving interventions.
...
#strong[Level 0: Descriptive]---"What happened?" Dashboards, reports, and statistics
provide backward-looking summaries.

#strong[Level 1: Predictive]---"What will happen?" ML models and forecasting assume a
stable world.

#strong[Level 2: Causal]---"Why?" and "What if we intervene?" DAGs and do-calculus
enable understanding of mechanisms.
...
Most organizations remain stuck at Level 1. Causal AI provides the methodology to
advance to Levels 2 and 3, where the greatest business value lies.
...
#strong[Digital transformation] uses digital technologies to reduce costs and improve
revenues through several mechanisms: integration of digital technology (embedding AI,
cloud, IoT, and automation into business areas), cultural and organizational change
(encouraging innovation and data-driven mindsets), customer-centric approaches (using
personalized experiences and AI-driven insights), process automation and optimization
(streamlining operations through AI and analytics), and data-driven decision making
(leveraging big data and real-time analytics for smarter, faster decisions).

// Slide: Why AI Projects Fail?

#strong[AI projects fail] primarily because they approach problems exclusively from
an ML perspective. Data scientists focus on model accuracy and technical metrics
while working in isolation from domain experts, business users, and data engineers.
This leads to a lack of understanding of problem context, producing irrelevant
solutions.

Key failure modes include: misalignment with business goals (models solve the wrong
problem), black-box models that fail to gain stakeholder trust, data issues that
undermine performance (poor quality, missing, or biased data), deployment and
integration challenges, and lack of feedback loops for monitoring post-deployment
performance.
...
1. #strong[Create a hybrid team] that represents all business aspects, uses a
  collaborative framework, and communicates with a single language (such as DAGs).
...
#strong[Step 1] focuses on defining intended outcomes. Given that most AI projects
(even 90%) fail due to misalignment between business goals and the project, this step
is critical. Teams must answer: What process are you analyzing? What happens if a
course of action is taken or not? What outcomes are positive, negative, unacceptable,
or optimal? What related data can be combined? What are possible and feasible
interventions? What confounding factors are correlated with outcomes and treatments?
What factors exist but cannot be accurately measured?
...
outcome. For example, in customer marketing for a product, possible interventions
include: introducing a new product, bundling multiple products (and determining
whether bundling improves or inhibits long-term sales), focusing advertising on
product quality versus alternatives, adding variations, discontinuing the product, or
divesting the product line entirely. Each intervention represents a different causal
pathway to the desired outcome.

// Slide: Step 3: What Factors Create Changes?

#strong[Step 3] identifies the factors that create changes, with particular attention
to #strong[confounders]---variables that obscure the relationship between treatment
and outcome by either muting or inflating the apparent effect.
...
environment or intervention byproducts), and the #strong[total causal effect] (the
combined effect of all factors modifying the outcome).

// Slide: Step 4: Build Causal DAG

#strong[Step 4] constructs the #strong[causal DAG] (Directed Acyclic Graph). Causal
models simplify complex systems without losing key relationships, focusing on
essential variables and interactions. Visual models abstract complexity into
interpretable formats and highlight the direction and strength of influence.

Simplicity serves multiple purposes: it aids communication between stakeholders,
reduces cognitive overload by excluding irrelevant details, guides data collection by
identifying impactful variables, and supports hypothesis testing through
counterfactual and intervention scenarios. The key is balance---too much simplicity
loses insight, while too much complexity loses clarity.

// Slide: Marketing Example: Price Intervention

Consider a marketing example where #strong[price is the intervention] and the goal is
to #strong[maximize revenue]. The team must decide: Increase price? By how much?
Decrease price? Change price one-time or over time? Develop individual pricing models
for each customer?
...
#strong[Step 5] involves data acquisition and integration. Organizations can reuse
data collected for correlation-based ML but must also collect data specifically for
causal AI. Data must be treated, conditioned, and transformed to match the refined
causal model.

#strong[Transformation] involves mapping observed variables to DAG nodes, encoding
categorical variables, handling missing data (through imputation or exclusion), and
normalizing or scaling values. The team must also control for bias and confounding.
The goal is to ensure that the data structure supports causal estimation---not just
predictive accuracy.

// Slide: Step 6: Model Modification

#strong[Step 6] refines the initial DAG to reduce bias and improve reliability. After
the initial design, variables must be clarified as confounders, mediators, or
outcomes.

Key pitfalls to avoid: do not control for mediators or colliders (which can distort
results), and do control for direct and indirect confounders (to prevent biased
estimates). Models are tested against both technical and business objectives. This
step determines the necessary controls to isolate causal effects, ensures causal
assumptions align with data and domain logic, and improves model interpretability.

// Slide: Step 7: Preparing for Deployment in Business

#strong[Step 7] transitions from experimentation to integration with decision-making
processes. This involves validating the model against real-world business outcomes
and ensuring stakeholders understand and trust the causal logic and assumptions.

Teams must develop user-friendly interfaces for business users, automate data
pipelines for timely updates, establish metrics for performance tracking and drift
detection, create feedback loops for post-deployment refinement, provide clear
documentation for auditors and users, and train stakeholders on interpreting results.
The ultimate goal is to deliver measurable business value.
...
- #strong[IT Professionals]: provide infrastructure, governance, and enterprise
  integration
- #strong[Project Managers]: coordinate collaboration, manage documentation, ensure
  strategic alignment

// Slide: Executing for a Hybrid Team Project

Executing a hybrid team project requires: establishing a collaborative approach that
aligns strategic goals with technical efforts and emphasizes early stakeholder
engagement; defining team goals using SMART objectives aligned with business
strategy; targeting a bounded, feasible, high-value use case that prioritizes early
wins for trust and momentum; defining the hypothesis by building a preliminary DAG
...
Understanding how ML models make decisions, whether they can be trusted, and whether
they are biased are critical questions for any organization deploying AI. Systems
must foster trust, transparency, and user confidence.

Management faces serious consequences from unexplainable AI: loss of trust,
regulation violations, fines, and lawsuits. A false negative in medical screening for
cancer, for instance, represents a catastrophic failure that demands explanation.
#strong[Humans in the loop] ensure that interpretation and context are considered
...
predictions by approximating the model locally with an interpretable surrogate,
perturbing input data and observing prediction changes.
...
#strong[Individual Conditional Expectation] (ICE) plots show the feature-prediction
relationship for individual instances rather than averages.

#strong[SHAP] (SHapley Additive exPlanations) quantifies each feature's contribution
to a specific prediction using game-theoretic principles.

// Slide: Causal AI in Interpretable AI

#strong[Causal AI] provides a natural framework for interpretability:

- DAGs present variables, relationships, and their strengths in a visual, intuitive
  format
...
The future of AI involves a #strong[convergence] of multiple approaches: Causal AI
moves from academia to commercial use, departing from purely data-driven systems to
reflect human thinking and decision-making.
