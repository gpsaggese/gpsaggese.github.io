...
def extended_kb_models() -> Tuple[List[sympy.Symbol], np.ndarray, np.ndarray]:
    """
    Build the extended `KB` and enumerate its 16 models.

    `KB = {Rain, Rain => Puddle, Puddle => WetGround, Sprinkler =>
    WetGround}`. `Sprinkler` is left unconstrained, so the `KB` is
    satisfied by models with `Sprinkler` true and by models with
    `Sprinkler` false.

    :return: tuple of
        - `[Rain, Puddle, WetGround, Sprinkler]`
        - the 16 enumerated models
        - the boolean mask marking `M(KB)`
    """
    rain, puddle, wet_ground, sprinkler = sympy.symbols(
        "Rain Puddle WetGround Sprinkler"
    )
    var_order = [rain, puddle, wet_ground, sprinkler]
    kb_sentences = [
        rain,
        sympy.Implies(rain, puddle),
        sympy.Implies(puddle, wet_ground),
        sympy.Implies(sprinkler, wet_ground),
    ]
    bits = enumerate_assignments(4)
    kb_mask = satisfying_mask(kb_sentences, var_order, bits)
    return var_order, bits, kb_mask


def true_verdicts() -> Dict[str, str]:
    """
    Decide every query by model checking, the entailment ground truth.

    :return: map from query label to verdict
    """
    var_order, bits, kb_mask = extended_kb_models()
    queries = dict(zip(["Rain", "Puddle", "WetGround", "Sprinkler"], var_order))
    verdicts = {}
    for label, query in queries.items():
        alpha_mask = satisfying_mask([query], var_order, bits)
        verdicts[label] = entailment_verdict(kb_mask, alpha_mask)
    return verdicts


def reasoner_verdicts(name: str) -> Dict[str, str]:
    """
    Run one of the three reasoners over the 4 queries.

    - `correct (model checking)`: the ground truth from `true_verdicts()`
    - `unsound (affirms the consequent)`: derives `Rain`, `Puddle`,
      `WetGround` correctly by modus ponens, then sees `WetGround` and the
      rule `Sprinkler => WetGround` and wrongly affirms `Sprinkler` too
    - `incomplete (single-hop modus ponens)`: applies modus ponens exactly
      once, deriving `Puddle` from `Rain`, but never re-applies it to derive
      `WetGround` from `Puddle`

    :param name: reasoner name shown in the dropdown
    :return: map from query label to the verdict the reasoner reports
    """
    if name == "correct (model checking)":
        verdicts = true_verdicts()
    elif name == "unsound (affirms the consequent)":
        derived = {"Rain", "Puddle", "WetGround", "Sprinkler"}
        verdicts = {
            label: "entailed" if label in derived else "unknown"
            for label in ["Rain", "Puddle", "WetGround", "Sprinkler"]
        }
    elif name == "incomplete (single-hop modus ponens)":
        derived = {"Rain", "Puddle"}
        verdicts = {
            label: "entailed" if label in derived else "unknown"
            for label in ["Rain", "Puddle", "WetGround", "Sprinkler"]
        ...
        raise ValueError("Unknown reasoner: %s" % name)
    return verdicts
...
def _draw_verdict_table(
    ...
    *,
    ...
    :param ax: axes to draw on
    :param true_verd: verdicts from model checking
    ...
    labels = list(true_verd.keys())
    n_rows = len(labels)
    ax.axis("off")
    ax.set_xlim(0, 3.0)
    ax.set_ylim(0, n_rows + 1)
    ax.set_title(title, fontsize=13, fontweight="bold")
    for col, header in enumerate(["query", "entailment", "reasoner"]):
        ax.text(
            col + 0.05,
            n_rows + 0.5,
            header,
    ...
    for index, label in enumerate(labels):
        y = n_rows - 1 - index
        kind = _classify_verdict(true_verd[label], reported[label])
        ax.add_patch(
            mpatches.Rectangle(
                (0, y),
                3.0,
        ...
        ax.text(0.05, y + 0.5, label, ha="left", va="center", fontsize=9)
        ax.text(1.05, y + 0.5, true_verd[label], ha="left", va="center", fontsize=9)
        ax.text(
            2.05,
            y + 0.5,
            reported[label],
...
def cell4_1_soundness_completeness(
    *,
    figsize: Optional[Tuple[float, float]] = None,
) -> None:
    """
    Compare a correct, an unsound, and an incomplete reasoner on the same `KB`.

    Interactive controls (ipywidgets):
    - `reasoner`: which of the three reasoners to run
    ...
    reasoner_dropdown = ipywidgets.Dropdown(
        options=[
            ...
            "unsound (affirms the consequent)",
            "incomplete (single-hop modus ponens)",
        ],
    ...
    def update_plot(change: Optional[Any] = None) -> None:
        ...
        with output:
            ...
            true_verd = true_verdicts()
            reported = reasoner_verdicts(name)
            false_positives = sum(
                1
                for label in true_verd
                if _classify_verdict(true_verd[label], reported[label])
                == "false positive"
            )
            false_negatives = sum(
                1
            ...
            _draw_verdict_table(
                ...
                true_verd,
                reported,
                title="KB = {Rain, Rain=>Puddle, Puddle=>WetGround, "
                "Sprinkler=>WetGround}",
            )
            fig.text(
                0.02,
                0.02,
                "reasoner: %s   false positives: %d   false negatives: %d"
                % (name, false_positives, false_negatives),
                fontsize=10,
                fontweight="bold",
            )
            plt.tight_layout()
    ...
    param_info = make_param_info(
        {
            "reasoner": "<code>correct</code> matches entailment exactly; "
            "<code>unsound</code> reports facts entailment does not "
            "support (false positives); <code>incomplete</code> misses "
            "facts entailment does support (false negatives)",
        }
    )
