...
- `prompt.generate_typst_book_chapter_common.md`: Typst rules specific to
  *this pipeline* (dissolving semantic tags, never fabricating a
  diagram's rendered figure) shared by both `typst_aima` generation paths
  below. It references `.claude/skills/typst.rules.md`: the general
  Typst syntax rules (document structure, emphasis, formulas, visuals)
  that apply to any `.typ` file, not just one generated by this script,
  and that Claude Code itself follows when writing or editing a `.typ`
  file directly. The reference is an inline
  `` `@.claude/skills/typst.rules.md` `` mention, expanded into that
  file's content by `hllm_cli.expand_referenced_files()` (see "Prompt
  building" below); the whole-document mode file below similarly
  references the companion template `.claude/templates/typst.template.typ`
...
def _expand_file_references(prompt: str) -> str:
    """
    Expand `` `@file` `` references in `prompt` into the referenced file's
    content.

    Unlike Claude Code (which resolves an `@file` mention itself, via its
    own file-reading tools), the LLM called by this script only ever sees
    the text handed to it, so a mention like
    `` `@.claude/skills/typst.rules.md` `` in one of the `prompt.*.md`
    files must be expanded here before the prompt is sent, or the LLM
    would see the literal, unresolved reference instead of its rules.

    :param prompt: system prompt text, possibly containing `@file`
        references
    :return: `prompt` with every `@file` reference replaced by that file's
        content (see `helpers.hllm_cli.expand_referenced_files()`)
    """
    import helpers.hllm_cli as hllmcli

    expanded_prompt = hllmcli.expand_referenced_files(prompt)
...
def _get_system_prompt(mode: str) -> str:
    ...
    system_prompt = "\n\n".join(parts)
    system_prompt = _expand_file_references(system_prompt)
...
# PNG (see `run_typst.py`), never by the LLM. Must stay in sync with the
# fence languages `render_images.py`'s `start_image_regex` accepts
# (`plantuml`, `mermaid`, `tikz`, `graphviz`, `latex`, `raw_latex`, `svg`,
# `image`): a language missing here silently falls through to the LLM as
# ordinary prose (source code and all) instead of being extracted as a
# diagram placeholder. The optional `(name)`/`[width=X%]` suffix
# `render_images.py` also accepts on the fence line (e.g.
# ```graphviz[width=80%]) is captured too, so it survives the placeholder
# round-trip (see `_render_diagram_placeholder()`).
_DIAGRAM_FENCE_RE = re.compile(
    r"```(?P<lang>plantuml|mermaid|tikz|graphviz|latex|raw_latex|svg|image)"
    r"(?P<suffix>(?:\([^)\n]*\))?(?:\[[^\]\n]*\])?)\n"
    r"(?P<code>.*?)\n```",
    re.DOTALL,
)
...
_DIAGRAM_BLOCK_RE = re.compile(
    r"(?P<indent>[ \t]*)```(?:plantuml|mermaid|tikz|graphviz|latex|raw_latex"
    r"|svg|image)(?:\([^)\n]*\))?(?:\[[^\]\n]*\])?\n"
    r"[\s\S]*?\n```\n"
...
def _describe_diagram(lang: str, code: str, slide_title: str) -> str:
    ...
    :param lang: fence language (see `_DIAGRAM_FENCE_RE`); only `graphviz`
        and `mermaid` support node-label extraction, every other language
        (`tikz`, `latex`, `raw_latex`, `plantuml`, `svg`, `image`) falls
        back to `slide_title`
...
# TODO(ai_gp): Move label: str, description: str before *
def _render_diagram_placeholder(
    lang: str, code: str, *, suffix: str = "", label: str, description: str
) -> str:
    """
    ...
    :param lang: fence language (see `_DIAGRAM_FENCE_RE`)
    :param code: diagram source code
    :param suffix: the fence line's original `(name)`/`[width=X%]` suffix,
        if any (e.g. "[width=62%]"), preserved verbatim so
        `render_images.py` still sees it
    ...
    *,
    ...
    def _repl_diagram(m: "re.Match[str]") -> str:
        ...
        placeholders[token] = _render_diagram_placeholder(
            lang, code, suffix=suffix, label=label, description=description
        )
...
def _render_book_chapter(
    ...
    *,
    ...
    if mode == "typst_aima":
        ...
        # <label>` calls. Without it, any diagram (`graphviz`, `mermaid`,
        # `tikz`, `latex`, `raw_latex`, `plantuml`, `svg`, `image`) in the
        # chapter never gets a real Typst label, so a `@fig:...`
        # cross-reference to it fails to compile with "label does not
        # exist".
        cmd = (
