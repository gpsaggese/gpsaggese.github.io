...
- Compiles a book chapter `.typ` file (generated by `gen_book_chapter.py
  --mode typst_aima`) to PDF via `run_typst.py`
- Writes the PDF to the staging dir `{DIR}/book.tmp/`, so a preview render
  never touches the PDF checked into `{DIR}/book/`, mirroring how
  `gen_slides.py` stages a slide PDF in `{DIR}/lectures_pdf.tmp/`
  - `-i`/`--input`: either `<dir>/<lesson>` (e.g., `msml610/03.2`) or a direct
    path to a book chapter `.typ` file
  - `-f`/`--files`: space-separated list of lecture specifications to render
    in one run
  - `--daemon` watches the `.typ` file and re-renders the PDF on change (only
    supports a single lecture)
  - `--run_typst_args` passes a verbatim options string through to
    `run_typst.py`
