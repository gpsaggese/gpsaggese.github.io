...
- Each course/book directory that the scripts read from and write to (e.g.,
  ...
  - `book.tmp/`: directory storing temporary PDF renders of book chapters
    - E.g., `render_book_chapter.py`
  ...
  - `test/`: unit tests for the scripts in this directory - One `test_<module>.py`
    per source module // TODO(ai_gp2): Make sure all dirs have this layout
...
| Generation    | `render_book_chapter.py`            | Render a book chapter `.typ` file to PDF, staged in `book.tmp/`              |
...
| `run_typst.py`        | Compiles a Typst `.typ` file to PDF; used by render_book_chapter.py    |
...
- Displays results in a formatted table supporting markdown (default), TSV, and CSV
  output formats
...
- `--dir DIR`: counts pages in all PDF files matching `Lesson*.pdf` in `DIR` (e.g.,
  the `{DIR}/lectures/` dir with slides generated by `gen_slides.py`, or the
  `{DIR}/book/` dir with book chapter PDFs generated by `gen_lecture_commentary.py`)
...
- Generates lecture slide PDFs from source files using `notes_to_pdf.py` to convert
  markdown to PDF, writing output to `<dir>/lectures/`
  - `-i`/`--input`: either `<dir>/<lesson>` (e.g., `data605/08.1`) or a direct path
    to a `Lesson*.txt` file
...
- Render with the Typst engine and pass an extra option through to `notes_to_pdf.py`:
...
- Automatically creates intro and outro sections, combines all sections, and lints
  the final output
...
- Generates a commented PDF ("lecture commentary") from a lecture source file in one
  step: converts the lecture source to a temporary PDF with `notes_to_pdf.py`,
  extracts slide images from the PDF and generates per-slide LLM commentary, renders
  the result to PDF with `pandoc`, and opens it in Skim
- Extracts title from markdown file (e.g., from `\text{\blue{Lesson 2.1: Git}}`) and
  adds YAML preamble for pandoc metadata
- Validates that the number of slides in markdown matches the number of PNG files
  (expects `num_slides + 1 = num_pngs` to account for title slide)
- Properly aligns title slide (first PNG) with content slides (remaining PNGs) to
  ensure header, slide image, and commentary are synchronized
...
- Compiles a book chapter `.typ` file (generated by
  `gen_book_chapter.py --mode typst_aima`) to PDF via `run_typst.py`
- Writes the PDF to the staging dir `{DIR}/book.tmp/`, so a preview render never
  touches the PDF checked into `{DIR}/book/`, mirroring how `gen_slides.py` stages a
  slide PDF in `{DIR}/lectures_pdf.tmp/`
  - `-i`/`--input`: either `<dir>/<lesson>` (e.g., `msml610/03.2`) or a direct path
    to a book chapter `.typ` file
  - `-f`/`--files`: space-separated list of lecture specifications to render in one
    run
  - `--daemon` watches the `.typ` file and re-renders the PDF on change (only
    supports a single lecture)
  - `--run_typst_args` passes a verbatim options string through to `run_typst.py`

### Examples

- Render the book chapter for a single lesson:
...
- Automatically formats output using `lint_text.py` with prettier (use `--no_lint` to
  skip)
...
- Scans `{DIR}/lectures_source/` for lesson files and generates an HTML page linking
  each lesson to its generated artifacts:
  - Slides PDF: `{DIR}/lectures_pdf/{LESSON}.pdf`
  - Lecture commentary: `{DIR}/lectures_commentary/{LESSON}.book_chapter.html` and
    `.book_chapter.pdf`
  - Lesson recap: `{DIR}/lectures_recap/{LESSON}.recap.md`
- Each existing artifact is linked to its GitHub URL (via `to_github.py`) instead of
  a local path, so the page works when shared outside a local checkout
...
- By default stops with an error as soon as an artifact is missing
  - `--do_not_fail_on_warnings` logs missing artifacts as warnings instead and lists
    the lesson without the missing link
...
- Generate the links page while tolerating missing artifacts, linking to `master`:
...
- Extracts individual slides from a markdown file and processes each with an LLM
  prompt selected by `--action`
  ...
  - `--no_abort_on_error` continues processing remaining slides if one LLM call fails
...
- Checks and fixes text (spelling, grammar, formatting) in lecture slides, in place,
  by shelling out to `process_slides.py --action text_check_fix --use_llm_transform`
...
- Reduces and simplifies lecture slides in place using LLM, by shelling out to
  `process_slides.py --action slide_reduce --use_llm_transform`
- `--process_slides_args` passes a verbatim options string through to
  `process_slides.py`
...
- Replaces `**Tag**` bold labels with `@Tag@` for the canonical set of slide/lecture
  tags (see "### Tags" in `.claude/skills/slides.rules.md`)
...
- Extract all pages from a PDF with default settings:
  ...
  Output files created: `output/slides001.png`, `output/slides002.png`, ...,
  `output/slides008.png`
...
- Generate images for the predefined MSML610 workload (note: the code checks for the
  literal string `"MSLM610"`):
...
- Transforms lecture slides using LLM with specified rules, reading slides from
  lesson files and writing the transformed slides back to the file or output
...
- Integrates with `hllm_cli._process_batches` for batch processing **Command Line
  Arguments:**
...
- Orchestrates the generation of multiple outputs from lecture source files for
  educational materials; the main entry point for processing lecture content into
  various formats
...
- Includes dry-run mode for previewing commands **Available Actions:**
...
- `generate_lecture_commentary`: Generate book chapter PDF (via
  `gen_lecture_commentary.py`) from lecture content, written to `<class>/book/`
...
- `generate_toc`: Extract table of contents (headers) from all lectures and create a
  consolidated course syllabus **Lecture Pattern Examples:**
- Single lecture: `01.1`
- Wildcard pattern: `01*`
- Multiple patterns: `01*:02*:03.1` (separated by colons)
- Continuous range: `01.1-03.2` (inclusive) **Command Line Arguments:**
- `--lectures`: Lecture(s) to process (optional; if omitted, processes all lessons in
  `--class`)
...
- `--cmd_opts`: Extra options string passed through verbatim to the invoked commands
  (e.g., `gen_lecture_commentary.py`, `notes_to_pdf.py`, `process_slides.py`), to
  control their behavior from inside the loop
...
1. Parse lecture patterns or ranges from command line arguments (e.g., '01*', '01.1',
   '01*:03\*', '01.1-03.2')
...
- Generates PDF files for all lessons starting with 0 or 1 (e.g., 01.1, 01.2, 10.1,
  etc.) in `data605/lectures/`:
...
- Use `--cmd_opts` to pass extra flags through to the underlying
  `gen_lecture_commentary.py` invocation for every lesson in the loop, e.g., force
  regeneration and open each PDF as it's produced:
...
- `for_loop_lessons.py --action improve_slide` is **not yet implemented** and aborts
  with an error; use the direct script instead:
...
- Output: `<class>/all_tocs.md` containing all lecture headers organized
  hierarchically
