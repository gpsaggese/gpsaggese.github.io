...
\begin{figure}[H]
  ...
  {Y}_{A}$ and $\bar{Y}_{B}$ reflect skill differences. Actual outcomes
  $Y_{A}$ and $Y_{B}$ deviate due to luck. Agent B with lower skill can outperform
  Agent A through favorable luck, though this becomes less likely as
  skill differences increase.}
...
\begin{figure}[H]
  ...
  ($\rho_{B}= 0.60$), with limited role for agency ($\rho_{A}= 0.20$). The
  high mobility society (right) exhibits lower circumstantial dependence
  ($\rho_{B}= 0.30$) and greater agency contribution ($\rho_{A}= 0.40$).
  Both societies contain irreducible luck components from random shocks.}
  \label{fig:variance}
\end{figure}