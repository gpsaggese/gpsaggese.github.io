# `racket_strategy_PR6` Implementation Spec: Example Notebook with Results

- `PR6` of `plan.racket_strategy.md`: turn the hand-worked example of paper
...
- Roadmap position: depends on `PR5`; `PR7` reports its outputs in the paper
...
- Results may contradict the toy example (e.g., Monte Carlo $P_{\mathrm{in}}$
  near 1 for all interior cells): report them as found and feed them into `PR7`
