...
import ck_marketing.one_off_scripts.CmampTask11363_Target_companies_list as cmoosctcl
"""
    ...
    for company in soup.find_all("h3"):
        ...
        for text in paragraphs:
            if "Headquarters:" in text:
                headquarters = text.split("Headquarters:", maxsplit=1)[
                    -1
                ].strip()
                found_match = True
            ...
            if "Glassdoor Score:" in text:
                glassdoor_score = text.split("Glassdoor Score:", maxsplit=1)[
                    -1
                ].strip()
                found_match = True
        if found_match:
...
if __name__ == "__main__":
    ...
    if not ai_companies_df.empty:
        ...
        hgofiapi.write_to_google_sheet(creds, ai_companies_df, SHEET_NAME)
