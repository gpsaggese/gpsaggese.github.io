...
import ck_marketing.workflows.workflows2 as cmawowor
"""
...
import ck_marketing.plugins.dropcontact.drop_contact_api as cmpddcoap
...
class GoogleSheetsHelper:
    ...
    def create_new_sheet_from_df(
        ...
        :param tab_name: name of the new Google Sheet.
        ...
        file_id = hgodrapi.create_empty_google_file(
            "sheet", tab_name, drive_folder_id
        )
        hdbg.dassert_ne(
            file_id,
            0,
            "tab_name='%s' in drive_folder_id='%s'",
            tab_name,
            drive_folder_id,
        )
    ...
    def write_results(
        ...
        new_tab_name: str = "hunter_results",
    ) -> None:
        """
        ...
        :param results_df: The DataFrame containing the results to
            write.
        :param new_tab_name: The name of the new worksheet where
        ...
        try:
            new_worksheet = sheet.add_worksheet(
                title=new_tab_name, rows="100", cols="20"
            )
...
# TODO(gp): Move it to the right file.
def hunter_drop_emails(
    ...
    dropcontact_results_df = cmpddcoap.get_email_from_dropcontact(
