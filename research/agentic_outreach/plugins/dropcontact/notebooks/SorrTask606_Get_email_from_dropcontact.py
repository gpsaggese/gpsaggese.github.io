...
import ck_marketing.plugins.dropcontact as mrkdrop
...
# tab_name = "Search4.FinTech_VC_in_US.SalesNavigator"
tab_name = "https://docs.google.com/spreadsheets/d/1FqXXx7NfGfO8xDjbNlANqWJx0wmeXX95BaXy115W-9c/edit#gid=41878666"
#
spread = gspread_pandas.Spread(tab_name, creds=creds)
...
# %% run_control={"marked": true}
def df_to_gsheet(tab_name: str, df: pd.DataFrame) -> None:
    # Write to the sheet.
    # Make sure the sheet "email"(tab_name) exists in the Google Sheet.
    tab_name = "email"
    spread2 = gspread_pandas.Spread(
        tab_name, sheet=tab_name, create_sheet=True, creds=creds
    )
    spread2.df_to_sheet(df, index=False)


#
df_to_gsheet(tab_name, email_df)
