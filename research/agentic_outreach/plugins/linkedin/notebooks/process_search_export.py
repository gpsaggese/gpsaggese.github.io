...
# %%
tab_name = f"{search_name}.step1.search_export"
spread = gspread_pandas.Spread(tab_name)
...
if cols[0] != "profileUrl":
    cols.remove(
        "profileUrl"
    )  # Remove the "profileURL" from its current position.
...
tab_name = f"{search_name}.step2.search_export_filtered"
