...
current_folder_id = hgofiapi.create_google_drive_folder(
    search_name, parent_folder_id
)

# %%
# Create empty gsheets in the new created folder.
for tab_name in gsheets_name:
...
phantom.download_result_csv_by_phantom_id(
    search_phantom_id, search_result_csv_path
)

# %%
# Download profile result csv.
phantom.download_result_csv_by_phantom_id(
    profile_phantom_id, profile_result_csv_path
)
...
def df_to_gsheet(tab_name: str, df: pd.DataFrame) -> None:
    ...
    gsheet = gspread_pandas.Spread(tab_name, create_sheet=True, creds=creds)
    gsheet.df_to_sheet(df, index=False)
    _LOG.info("Save to gsheet %s", tab_name)
