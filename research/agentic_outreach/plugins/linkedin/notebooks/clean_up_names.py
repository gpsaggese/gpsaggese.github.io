...
def process_names(row) -> pd.Series:
    first_name_cleaned, first_nickname = cmplliut.clean_first_last_name(
        row["first_name"], last_name_only=False
    )
    last_name_cleaned, last_nickname = cmplliut.clean_first_last_name(
        row["last_name"], last_name_only=True
    )
    # Choose the nickname with precedence given to the first name.
    return pd.Series(
        [first_name_cleaned, last_name_cleaned, first_nickname, last_nickname]
    )
...
df_ai_transform["input"] = (
    "First Name: "
    + df_ai_transform["first_name"]
    + "  Last Name: "
    + df_ai_transform["last_name"]
)
...
for model in models:
    df_ai_transform[f"{model}_first_name"] = df_ai_transform[model].str.extract(
        r"First name:\s*(.*?),"
    )
    df_ai_transform[f"{model}_last_name"] = df_ai_transform[model].str.extract(
        r"Last name:\s*(.*),"
    )
    df_ai_transform[f"{model}_nickname"] = df_ai_transform[model].str.extract(
        r"Nickname:\s*(.*)"
    )
