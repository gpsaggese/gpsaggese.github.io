...
def _cleanup(regex: str, decoded_query: str) -> str:
    """
    Clean up the decoded query by removing the regex pattern.

    :param regex: The regex pattern to remove.
    :param decoded_query: The decoded query to clean up.
    :return: The cleaned up query.
    """
    hdbg.dassert_isinstance(regex, str)
    hdbg.dassert_isinstance(decoded_query, str)
...
def parse_query(query: str) -> Dict[str, Any]:
    ...
    _LOG.debug("%s", hprint.to_str("decoded_query"))
    ...
    _LOG.debug("%s", hprint.to_str("filter_matches"))
    decoded_query = _cleanup(regex, decoded_query)
    #
    ...
    for filter_type, values in filter_matches:
        # Match entries with optional id field.
        # Handles both: (id:123,text:Foo,selectionType:INCLUDED)
        # and: (text:Foo,selectionType:INCLUDED)
        regex = r"\((?:id:(.*?),)?text:(.*?),selectionType:(\w+)\)?,?"
        value_matches = re.findall(regex, values)
        _LOG.debug("%s", hprint.to_str("filter_type value_matches"))
        for value_id, text, selection_type in value_matches:
            _LOG.debug("  %s", hprint.to_str("value_id text selection_type"))
            text = urllib.parse.unquote(text)
            filter_entry = {"text": text, "selectionType": selection_type}
            # Only add id if it's present.
            if value_id:
                filter_entry["id"] = value_id
            filters[filter_type].append(filter_entry)
...
def query_to_str(
    parsed_query: Dict[str, Any], *, sort_keys: bool = False
) -> str:
    """
    Convert the parsed query to a string.

    :param parsed_query: The parsed query to convert to a string.
    :param sort_keys: Whether to sort the keys of the parsed query.
    :return: The string representation of the parsed query.
    """
...
def generate_query(parsed_query: Dict[str, Any]) -> str:
    ...
    :param parsed_query: The parsed query dictionary.
    :return: The generated URL query string.
    """
    ...
    for filter_type, values in filters.items():
        ...
        for value in values:
            ...
            if "id" in value and value["id"]:
                ...
                value_str = (
                    f"text:{value_text},selectionType:{value_selection_type}"
                )
    ...
    if session_id:
        session_id_str = "&sessionId=" + urllib.parse.quote(
            session_id, safe="()"
        )
    else:
        session_id_str = ""
    ...
    # Combine all parts into final URL.
