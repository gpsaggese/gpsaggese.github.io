...
# TODO(gp): Probably redundant with other filtering functions in hpandas.
def filter_df(
    ...
    :param mode: "keep" for retaining and "remove" for removing those rows.
    ...
    if mode == "keep":
        ...
        _LOG.warning(
            "%d entries were removed", initial_length - len(filtered_df)
        )
    elif mode == "remove":
        filtered_df = dataframe[
            ~dataframe[column].apply(lambda x: bool(regex_pattern.search(x)))
...
def clean_first_last_name(name: str, *, is_last_name: bool) -> Tuple[str, str]:
    ...
    :param is_last_name: True if the input is the last name, False if the
        input is the first name.
    :return: tuple containing the cleaned first/last name and nickname.
    """
...
def clean_and_track_name_changes(
    ...
    *,
    ...
    :return: df with cleaned first and last names and column indicating if
        they were modified.
    """
...
def _contains_pronoun_or_slash(content: str) -> bool:
...
def _clean_full_name(name: str) -> str:
    ...
    if nickname_matches:
        ...
        if nicknames:
            ...
            if _contains_pronoun_or_slash(nickname.lower()):
...
def merge_clean_names_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    hdbg.dassert_is_subset(col_names, df.columns)
    # Merge cleaned names back into original columns.
...
def get_clean_name_stats(df: pd.DataFrame) -> pd.DataFrame:
    ...
    stats_df["empty_first_name"] = hpandas.to_perc(
        df["cleaned_first_name"] == ""
    )
...
def filter_VCs(df: pd.DataFrame, title_col_name: str) -> pd.DataFrame:
    ...
    filtered_df_vc = filter_df(df, title_col_name, words, mode="keep")
    filtered_df_non_vc = filter_df(df, title_col_name, words, mode="remove")
