...
#           'emails': [{'confidence': 99,
#                       'department': 'finance',
#                       'first_name': 'Ryan',
#                       'last_name': 'Hill',
#                       'linkedin': 'https://www.linkedin.com/in/ryan-hill-mba-9a4aa69',
#                       'phone_number': None,
#                       'position': 'Chief Financial Officer',
#                       'position_raw': 'Head of Finance and Accounting',
#                       'seniority': 'executive',
#                       'sources': [{'domain': 'linkedin.com',
#                                    'extracted_on': '2025-01-11',
#                                    'last_seen_on': '2025-01-11',
#                                    'still_on_page': True,
#                                    'uri': 'https://www.google.com/search?q=site:linkedin.com%20ryan%20hill%20braunhagey'}],
#                       'twitter': None,
#                       'type': 'personal',
#                       'value': 'hill@braunhagey.com',
#                       'verification': {'date': '2025-11-16',
#                                        'status': 'valid'}},
#           ...
#           'linked_domains': [],
#           'organization': 'BraunHagey & Borden',
#           'pattern': '{last}',
#           'webmail': False},
#  'meta': {'limit': 5,
#           'offset': 0,
#           'params': {'company': 'BraunHagey Borden LLP',
#                      'department': None,
#                      'domain': None,
#                      'seniority': None,
#                      'type': None},
...
def hunterio_domain_search(
    company: str, *, limit: int = 1, print_results: bool = False
) -> Dict[str, Any]:
    """
    Run a domain search on Hunter.io and return the people found in that domain.

    :param company: The company to search for.
    :param limit: The limit of the search.
    :param print_results: Whether to print the data.
    :return: A dictionary with the results of the domain search.
    """
    HUNTER_API_KEY = os.environ["HUNTER_API_KEY"]
    url = f"https://api.hunter.io/v2/domain-search?company={company}&limit={limit}&api_key={HUNTER_API_KEY}"
    response = requests.get(url)
    response.raise_for_status()
    data = response.json()
    if print_results:
...
#           'sources': [],
#           'twitter': None,
#           'verification': {'date': '2025-12-12', 'status': 'valid'}},
...
def hunterio_email_finder(
    first_name: str, last_name: str, company: str, *, print_results: bool = False
) -> Dict[str, Any]:
    """
    ...
    :return: A dictionary with the results of the email finder.
    """
    HUNTER_API_KEY = os.environ["HUNTER_API_KEY"]
    url = f"https://api.hunter.io/v2/email-finder?company={company}&first_name={first_name}&last_name={last_name}&api_key={os.environ['HUNTER_API_KEY']}"
    response = requests.get(url)
    response.raise_for_status()
    data = response.json()
    if print_results:
    ...
    if val == "":
        ...
        if response.status_code != 200:
            print(response.text)
            if data.get("errors").get("id") == "blacklisted_domain":
                val = "_blacklisted_domain_"
        response.raise_for_status()
        data = response.json()
        val = data.get("data", {}).get("status", "_nan_")


def hunterio_email_verifier(
    email: str, *, print_results: bool = False
) -> Dict[str, Any]:
    """
    Find an email address using Hunter.io.
    ...
    :return: A dictionary with the results of the email finder.
    """
    url = f"https://api.hunter.io/v2/email-verifier?email={email}&api_key={os.environ['HUNTER_API_KEY']}"
    response = requests.get(url)
    response.raise_for_status()
    data = response.json()
    if print_results:
...
# ) -> Dict[str, Any]:
#     """
#     Enrich a contact with information from Hunter.io.
...
#     :param is_company: Whether the company is a company.
#     :param issue_warnings: Whether to issue warnings or assert on errors.
#     :return: A dictionary with the enriched contact.
#         ```
#         {
#             'data': {
#                 'email': <str>,
...
#         tag = "domain"
#     HUNTER_API_KEY = os.environ["HUNTER_API_KEY"]
#     url = f"https://api.hunter.io/v2/email-finder?{tag}={company}&first_name={first_name}&last_name={last_name}&api_key={HUNTER_API_KEY}"
#     # Parse the results.
#     response = requests.get(url)
#     response.raise_for_status()
#     data = response.json()
...
# # Decorator
# # TODO(gp): Use hcache_simple.simple_cache().
...
def hunterio_verify_email(email: str) -> str:
    """
    ...
    :return: the verification status from hunter.io
        ...
        or a special value
        - `_missing_info_`: not all the info is available to perform a query
        - `_blacklisted_domain_`: the email is on a blacklisted domain
        - `_nan_`: hunter.io did not respond
        - `_error_`: hunter.io returned an error
        - `_timeout_`: the email verification is still in progress
    """
    ...
    if val == "":
        ...
        if response.status_code != 200:
            ...
            #     },
            #     "message": "The email verification is still in progress. To avoid the request running for too long we return HTTP 202 responses. Feel free to make the same verification again as often as you want, it will only count as a single request until we return the response."
            #   }
            # }
            meta = data.get("meta")
            if meta and meta.get("message").startswith(
                "The email verification is still in progress."
            ):
                return "_timeout_"
            errors = data.get("errors")
            if errors and errors[0].get("id") == "blacklisted_domain":
                return "_blacklisted_domain_"
            print(response.text)
        response.raise_for_status()
        data = response.json()
        val = data.get("data", {}).get("status", "_nan_")
...
hunterio_cached_verify_email = hcacsimp.simple_cache()(hunterio_verify_email)
...
def hunterio_verify_emails_from_df(
    ...
    *,
    ...
    """
    Verify emails in each row of a DataFrame using Hunter.io.

    :param df: contains the column with email addresses to verify.
    :param email_col: column name with email addresses to verify.
    :param incremental: whether to only verify emails that are not already verified
        according to the `target_verification_col`
    :param target_verification_col: column name to store the verification results.
    :return: original df with an additional column with verification status.
    """
    ...
    for idx, row in tqdm(df.iterrows(), total=df.shape[0]):
        if incremental and row[target_verification_col] != "":
            continue
        # Verify the email.
        val = hunterio_cached_verify_email(row[email_col])
        # Update.
        df.loc[idx, target_verification_col] = val
        num_updates += 1
    # Report stats.
    _LOG.info(
        "Updated %s rows with verification status",
        hprint.perc(num_updates, df.shape[0]),
    )
    return df


def get_hunterio_email_stats(
    diagnostics_df: pd.DataFrame, col_name: str, is_email_verification: bool
) -> Dict[str, Any]:
    """
    Get the statistics for an email column or an email verification column.

    :param diagnostics_df: The diagnostics DataFrame.
    :param col_name: The column name to get the statistics for.
       E.g., "hunterio.email_verification" or "hunterio.email".
    :param is_email_verification: Whether the column is an email verification
       column. If `True`, the tokens are all the values. If `False`, the tokens are
       all the values that do not contain an `@`.
    :return: A dictionary with the statistics.
        ```
        {'non-empty': 51,
         'non-empty [%]': '100.00%',
         'token counts': {'not reachable': 40,
                         '_blacklisted_domain_': 1,
                         'accept_all': 12,
                         'invalid': 10,
                         'valid': 28},
         'token counts [%]': {'not reachable': '78.43%',
                             '_blacklisted_domain_': '1.96%',
                             'accept_all': '23.53%',
                             'invalid': '19.61%',
                             'valid': '54.90%'},
         'token values': ['valid', 'invalid', 'accept_all', '_blacklisted_domain_'],
         'tokens': 51,
         'tokens [%]': '100.00%'}
        ```
    """
    ...
    dict_ = dict(
        sorted(dict_.items(), key=lambda item: item[1], reverse=reverse)
    )
    ...
    if is_email_verification:
        res["token counts"]["not reachable"] = res["token counts"].get(
            "accept_all", 0
        ) + res["token counts"].get("valid", 0)
        res["token counts [%]"]["not reachable"] = hprint.perc(
            res["token counts"]["not reachable"],
...
def get_hunterio_email_stats(
    ...
    :param col_name: The column name to get the statistics for.
       E.g., "hunterio.email_verification" or "hunterio.email".
    :param is_email_verification: Whether the column is an email verification column. If `True`, the tokens are all the values. If `False`, the tokens
        are all the values that do not contain an `@`.
    :return: A dictionary with the statistics.
        ```
        {'non-empty': 51,
         'non-empty [%]': '100.00%',
...
def remove_hunterio_invalid_emails_from_df(
    df: pd.DataFrame,
    *,
    email_verification_col: str = "hunterio.email_verification",
    print_stats: bool = False,
) -> pd.DataFrame:
    hdbg.dassert_in(email_verification_col, df.columns)
    #
    stats_dict = get_hunterio_email_stats(
        df, email_verification_col, is_email_verification=True
    )
    if print_stats:
        pprint.pprint(stats_dict)
    # Remove invalid emails.
    mask = df[email_verification_col].isin(["valid", "accept_all"])
    print("Kept %s emails", hprint.perc(mask.sum(), len(mask)))
    df = df[mask]
    return df
    ...
    for idx, row in tqdm(df.iterrows(), total=df.shape[0]):
        ...
        if mode == "FromScratch":
            ...
            if email_val != "":
                ...
                email_verification_val = hunterio_cached_verify_email(email_val)
            else:
                # If there is no email address, find one.
                email_val, email_verification_val = _get_email_and_verification(
                    first_name,
...
def hunterio_enrich_contact_df(
    df: pd.DataFrame,
    first_name_col: str,
    last_name_col: str,
    company_col: str,
    ...
    *,
    ...
    cols_map = cmwdlout._resolve_None_in_cols_map(cols_map)
...
def get_diagnostic_df(
    ...
    for col_name in contact_df.columns:
        if col_name.startswith(prefix):
            ...
            _LOG.info("Found: %s", hprint.to_str("col_name prefix_col_name"))
...
def get_stats(diagnostics_df: pd.DataFrame) -> Dict[str, Any]:
    """
    Get the statistics for a contact df.
