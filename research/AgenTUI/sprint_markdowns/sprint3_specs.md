...
- **Gemini CLI composer** (`gemini-cli/packages/cli/src/ui/components/InputPrompt.tsx` + `shared/text-buffer.ts`)
...
- **Codex CLI composer** (`codex/codex-rs/tui/src/public_widgets/composer_input.rs` & `bottom_pane/mod.rs`)
...
- **Codex startup box** (`codex/codex-rs/tui/src/app.rs`, render helpers in `tui/src/render/mod.rs`)
