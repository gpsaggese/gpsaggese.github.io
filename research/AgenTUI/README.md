...
- **DeepAgents integration** with OpenAI `gpt-4o-mini` by default. Override with `OPENAI_MODEL`.
...
```
OPENAI_API_KEY=sk-...
OPENAI_MODEL=gpt-5-mini   # optional; defaults to gpt-4o-mini when unset
SYSTEM_PROMPT=...         # optional; defaults to DEFAULT_SYSTEM_PROMPT
```

Copy `.env.example` to get started. Only `OPENAI_API_KEY` is required.
...
| `npm run build` | Type-check and emit JS into `dist/`, then copy assets.
| `npm start` | Run the compiled CLI (`dist/src/cli.js`).
...
| `npm run notebook:test` | Notebook pipeline smoke test (`scripts/notebook-smoketest.ts`).
...
- `/model [name]` – switch models. Supported ids live in `src/models.ts`: gpt-5, gpt-5-mini, gpt-5-nano, gpt-5.1, gpt-4o, gpt-4o-mini, gpt-4o-mini-2024-07-18, gpt-4.1, gpt-4.1-mini, o1-preview, o1-mini. Run `/model` with no args to open the inline picker.
...
| `src/config/` | Zod-validated config loader plus tests. `index.ts` exports `loadConfig` used by both TUI and CLI.
...
| `src/commands/` | Slash command parser (`index.ts`) + tests.
