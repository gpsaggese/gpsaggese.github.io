...
source $SCRIPT_DIR/docker_gpu_utils.sh
...
DOCKER_RUN_OPTS=$(get_mctx_docker_run_opts)
...
if [[ "$DOCKER_ENGINE_CURRENT" == "apple" ]]; then
    ...
    run "$DOCKER_CMD run --rm -d $DOCKER_CMD_OPTS $DOCKER_RUN_OPTS $FULL_IMAGE_NAME $CMD"
    sleep 3
    ...
    run "$DOCKER_CMD $DOCKER_CMD_OPTS $DOCKER_RUN_OPTS $FULL_IMAGE_NAME $CMD"
fi
