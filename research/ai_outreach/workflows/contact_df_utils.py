...
def clean_up_contact_df(
    ...
    if debug == phase:
        IPythond.display(df[~valid_mask])
        assert 0
    df = df[valid_mask]
    ...
    is_email = df["linkedin_url"].apply(is_valid_email)
    result[phase] = hprint.perc(is_email.sum(), num_before)
    if debug == phase:
        IPythond.display(df[is_email])
        assert 0
    ...
    if debug == phase:
        IPythond.display(df[is_website])
        assert 0
    srs = df["linkedin_url"].copy()
    ...
    if debug == phase:
        IPythond.display(df[invalid_mask])
        assert 0
...
def sanity_check_contact_df(
    ...
    *,
    ...
    df = df[col_names]
    contact_schema = aowdlout.get_contact_df_schema()
    ...
    if debug == phase and not valid_mask.all():
        print(f"Invalid {(~valid_mask).sum()} linkedin_url values")
        if verbose:
            IPythond.display(df[~valid_mask])
        assert 0
    ...
    if debug == phase and not valid_mask.all():
        print(f"Invalid {(~valid_mask).sum()} email values")
        if verbose:
            IPythond.display(df[~valid_mask])
        assert 0
    ...
    if debug == phase and not valid_mask.all():
        print(f"Invalid {(~valid_mask).sum()} email_verification values")
        if verbose:
            IPythond.display(df[~valid_mask])
        assert 0
...
def print_contact_df_stats(
    ...
    *,
    ...
    if debug == phase and (num_rows_no_dups != df.shape[0]):
        ...
        IPythond.display(df[duplicated])
        assert 0
    df = df_no_dups
    ...
    if debug == phase and (num_rows_only_ascii != df.shape[0]):
        IPythond.display(df[valid_ascii])
        assert 0
    df = df[valid_ascii]
...
def print_contact_df_detailed_stats(
    ...
    """
    Show a heatmap of the stats, understanding the semantics of a contact df.

    Similar to `get_column_stats()`.
    """
    stats_df_list: List[pd.DataFrame] = []

    def _perc(a: float, b: float) -> float:
        return float(hprint.perc(a, b, only_perc=True, use_float=True))
    ...
    for col_name in df.columns:
        ...
        stats_df_list.append(pd.DataFrame(stats_df_tmp).T)
    #
    ...
    if mode == "only_pct":
        ...
        IPythond.display(hpandas.heatmap_df(stats_df))
    else:
...
def get_column_stats(
    ...
    *,
    ...
    result["unique_tokens"] = f"{len(unique_tokens)} {' '.join(unique_tokens)}"
    # Return values.
    if mode in ("str", "print_str"):
        txt = [f"{k}={v}" for k, v in result.items()]
        txt = "\n".join(txt)
        if mode == "str":
            value = txt
        elif mode == "print_str":
...
def infer_category(
    ...
    *,
    ...
    def _append_mask(mask: Any, tag: str) -> None:
        mask = pd.Series(mask, index=main_mask.index)
        masks[tag] = mask & main_mask
        stats_data.append([tag, sum(mask), 100.0 * sum(mask) / len(mask)])
...
def save_to_gsheet(
    ...
    *,
    ...
    hgodrapi._freeze_rows_in_gsheet(  # pylint: disable=protected-access
        credentials,
        spread_id,
    ...
    #
    hgodrapi._set_row_height_in_gsheet(  # pylint: disable=protected-access
        credentials,
        spread_id,
        height=20,
        tab_name=tab_name,
