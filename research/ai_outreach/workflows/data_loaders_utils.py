...
_contact_df_schema = [
    ...
    # See ai_outreach/plugins/pitchbook/position_departments.grouped.txt
    "type",
    # Bio of the person.
    "biography",
]
...
def _rename_columns_to_contact_schema(
    ...
    :return: A tuple containing the DataFrame with the renamed columns
        and the dictionary mapping the original column names to the new
        column names.
    """
    df = df.copy()
...
# - `sanity_check()` just checks that the data is in the correct format.
...
def normalize_contact_schema(
    ...
    """
    Normalize a dataframe to use the Contact schema, based on the column remap.
...
def fuzzy_column_matching(
    ...
    *,
    ...
    :return: Tuple of (matched_map, unmatched_keys, unmatched_cols)
    """
    ...
    for key, values in map2_.items():
        ...
        for val in values:
            for col in cols:
                if (
                    val.lower() == col.lower()
                    and key not in out_map
                    and col not in out_map.values()
                ):
                    out_map[key] = col
    ...
    for key, values in map2_.items():
        if key not in out_map:
            _LOG.log(log_level, "Key '%s' was not matched", key)
    ...
    for col in cols:
        if col not in out_map.values():
            _LOG.log(log_level, "Column '%s' was not matched", col)
