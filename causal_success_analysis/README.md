...
3. Run Jupyter Lab inside the container:
   ```bash
   docker run -p 8888:8888 causal_success_analysis
   ```
...
├── template.example.py                  # Simulation model (agents + events)
├── template.example.md                  # Markdown tutorial walkthrough
├── tutorial_github_data605_style.ipynb  # Jupyter notebook tutorial
├── docker_build.sh                      # Helper script to build Docker image
├── docker_jupyter.sh                    # Helper script to start Jupyter
├── SUBMISSION_CHECKLIST.txt             # QA checklist before PR
...
- Visualize wealth trajectories and Lorenz curves.  
- Extend causal analysis using DML and uplift modeling.  
...
- [ ] Simulation event dynamics under active development.  
- [ ] Causal ML modules to be integrated in the next phase.
