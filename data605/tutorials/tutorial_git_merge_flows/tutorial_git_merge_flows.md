...
- History stays non-linear: the merge commit has 2 parents, and `feature`'s 5
...
- History is linear, at the cost of 5 manual conflict resolutions instead of 1; this
  is the pattern behind "why does merging master take forever", when a branch has
  accumulated many small commits that overlap with upstream changes and gets rebased
  instead of merged
